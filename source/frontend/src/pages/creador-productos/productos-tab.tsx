...
import { CONTROL_LOTE_OPTIONS } from "@/lib/control-lote";
...
function CreateProductoDialog({
  ...
  const [activeFormTab, setActiveFormTab] = useState<"identificacion" | "almacenamiento" | "inventario">("identificacion");
    ...
    <>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Form Tabs */}
          <div className="tabs tabs-boxed bg-base-200/60 p-1 flex gap-1 rounded-xl">
            <button
              type="button"
              onClick={() => setActiveFormTab("identificacion")}
              className={cn(
            ...
            <button
              type="button"
              onClick={() => setActiveFormTab("almacenamiento")}
              className={cn(
                "tab flex-1 py-1.5 text-center text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all",
                activeFormTab === "almacenamiento"
                  ? "bg-primary text-primary-content shadow-sm"
                  : "text-base-content/60 hover:text-base-content/90"
              )}
            >
            ...
            <button
              type="button"
              onClick={() => setActiveFormTab("inventario")}
              className={cn(
                "tab flex-1 py-1.5 text-center text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all",
            ...
            {activeFormTab === "almacenamiento" && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="label py-0.5">
                      <span className="label-text text-sm font-medium">Clase de riesgo</span>
                    </label>
                    <select
                      className="select select-sm bg-base-100 border border-base-300 rounded-xl"
                      value={claseRiesgo ?? ""}
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <div className="form-control justify-center">
                    <label className="flex items-center gap-2 cursor-pointer">
                      ...
                      />
                      <span className="text-sm font-medium">Requiere cadena de frío</span>
                    </label>
                  </div>
                  <div className="form-control">
                    <label className="label py-0.5">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="form-control">
                    ...
                    <input
                      ...
                      value={form.promedio_uso_mensual_inicial}
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          promedio_uso_mensual_inicial: e.target.value,
                        }))
                      }
                      placeholder="Ej: 50"
                    />
                ...
                <div className="pt-2 flex items-center gap-3">
                  {form.imagen_data_url && (
                    <div className="w-10 h-10 rounded-lg overflow-hidden border border-base-300 shrink-0">
                      <img
  ...
  useEffect(() => {
    if (producto) {
      ...
      setForm({
        ...
        promedio_uso_mensual_inicial: producto.promedio_uso_mensual_inicial ? String(producto.promedio_uso_mensual_inicial) : "",
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div className="form-control">
                      ...
                      <input
                        type="text"
                        className="input input-bordered input-sm h-9 bg-base-200 text-base-content/50"
                        value={producto?.unidad_base?.nombre || ""}
                        disabled
                        title="La unidad base no se puede editar"
                      />
                    </div>
                  </div>
...
function ProductoDetail({ id }: { id: string }) {
  const [activeTab, setActiveTab] = useState<"detalles" | "empaques" | "ofertas">("detalles");
        ...
        {activeTab === "detalles" && (
          <div className="space-y-3">
            ...
            {producto.areas && producto.areas.length > 0 && (
              <div className="flex justify-between items-center py-1.5 border-b border-base-200/40">
                <span className="text-[11px] opacity-40 uppercase tracking-wider font-medium">
                  Área / Sección
                </span>
                <div className="flex flex-wrap gap-1 justify-end max-w-[70%]">
                  {producto.areas.map((a: { id: number; nombre: string }) => (
                    <Badge key={a.id} variant="secondary" className="text-[10px] py-0.5">
                      {a.nombre}
                    </Badge>
                  ))}
                </div>
