...
async function fetchGlobalResumenData(
  filters?: PdfOptions["filters"],
): Promise<GlobalAlertData> {
  ...
  const [activosResp, alertsFirst] = await Promise.all([
    api
      .get<GlobalStockResponse>("/stock", {
        params: { ...filters, per_page: 1, page: 1 },
      })
      .then((r) => r.data),
    api
      .get<GlobalStockResponse>("/stock", {
        params: { ...filters, con_alertas: true, per_page: 100, page: 1 },
      })
      .then((r) => r.data),
  ]);

  const allAlerts =
    alertsFirst.total_pages <= 1
      ? alertsFirst.data
      : [
          ...alertsFirst.data,
          ...(await Promise.all(
...
// ─── fetchStockForArea ─────────────────────────────────────────────────────
async function fetchStockForArea(
  areaId: number,
  filters?: PdfOptions["filters"],
): Promise<StockItem[]> {
  const params = {
...
function drawHeader(
  ...
  doc.setFontSize(7);
  doc.setTextColor(...C.grayMid);
  doc.text(`${usuarioNombre} · ${horaStr}`, W - MARGIN, HEADER_H / 2 + 5, {
    align: "right",
  });
...
function drawFooterFinal(
  ...
  // Texto derecha
  doc.text(`Página ${pageNum} de ${totalPages}`, W - MARGIN, H - 3, {
    align: "right",
  });
}
...
function drawResumen(
  ...
  doc.setFontSize(7.5);
  doc.setTextColor(...C.grayDark);
  doc.text(`${selectedAreas.length} secciones incluidas`, scopeX, y + 6, {
    align: "right",
  });
  ...
  doc.setTextColor(...C.black);
  doc.text(
    formatPrecio(valorTotalInventario, monedaCodigo),
    W - MARGIN,
    y + 9,
    { align: "right" },
  );
  ...
  function drawAlertCol(
    ...
    if (rows.length === 0) {
      ...
      doc.setFontSize(7);
      doc.setTextColor(...C.grayMid);
      doc.text("Sin alertas activas", x + colW / 2, rowsY + 4, {
        align: "center",
      });
      return;
    }
  ...
  drawAlertCol(colL, "Reposición", itemsSinStock.length + itemsBajo.length, [
    ...itemsSinStock.map((i) => ({
...
export async function exportarStockPDF(options: PdfOptions): Promise<void> {
  ...
  const now = new Date();
  const horaStr = now.toLocaleTimeString(APP_LOCALE, {
    hour: "2-digit",
    minute: "2-digit",
  });
