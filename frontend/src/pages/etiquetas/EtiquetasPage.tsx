...
export function EtiquetasPage() {
  ...
  const { data: lotes = [], isLoading: cargandoLotes } = useQuery({
    ...
    queryFn: () => api.get<LoteEnStock[]>('/lotes', {
      ...
      <div ref={containerRef} className="relative">
        <div className="relative">
          ...
          <input
            ...
            aria-activedescendant={activeIndex >= 0 ? `etiqueta-sugerencia-${activeIndex}` : undefined}
          />
        </div>
        ...
        {dropdownOpen && (
          <div className="absolute z-20 mt-1 w-full max-h-72 overflow-y-auto rounded-xl border border-base-300 bg-base-100 shadow-lg">
            {cargandoProductos && (
              <div className="px-4 py-3 text-sm text-base-content/50">Buscando…</div>
            )}
            {!cargandoProductos && dropdownItems.length === 0 && (
              <div className="px-4 py-3 text-sm text-base-content/50">Sin resultados.</div>
            )}
            {dropdownItems.map((p, i) => (
              ...
              >
                <span className="min-w-0 flex-1 truncate font-medium">{p.producto_nombre}</span>
                <span className="flex-shrink-0 text-xs text-base-content/50">
                  {formatCantidad(p.stock_total ?? 0, p.unidad, p.unidad_plural ?? undefined)} en stock
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between rounded-xl border border-base-200 bg-base-100 px-4 py-3">
            <div className="min-w-0">
              <p className="truncate font-semibold">{producto.producto_nombre}</p>
              <p className="text-xs text-base-content/50">{producto.codigo_interno}</p>
            </div>
            <button
              type="button"
              className="btn btn-ghost btn-sm btn-circle"
              aria-label="Cambiar producto"
              onClick={() => { setProducto(null); setSelectedLoteIds([]); inputRef.current?.focus() }}
            >
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <div>
            ...
            {cargandoLotes && <p className="text-sm text-base-content/50">Cargando lotes…</p>}
            {!cargandoLotes && lotes.length === 0 && (
              <p className="text-sm text-warning/80">Este producto no tiene lotes con stock.</p>
            )}
                ...
                return (
                  <label
                    ...
                    className={cn(
                      ...
                      checked ? 'border-primary/40 bg-primary/5' : 'border-base-200 bg-base-100 hover:border-base-300',
                    )}
                  >
                    <input
                      type="checkbox"
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-base-content/50">
                        Vence: {new Date(l.fecha_vencimiento + 'T00:00:00').toLocaleDateString('es-AR')}
                      </p>
                    </div>
                    <span className="flex-shrink-0 text-xs text-base-content/50">
                      {formatCantidad(l.stock_total ?? 0, producto.unidad, producto.unidad_plural ?? undefined)}
                    </span>
                  </label>
                )
              })}
          ...
          {lotesParaImprimir.length > 0 && (
            <LabelsSection key={selectedLoteIds.join(',')} lotesConfirmados={lotesParaImprimir} />
          )}
        </div>
      )}
    </div>
