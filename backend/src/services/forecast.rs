...
impl SbcCategory {
    pub fn as_str(self) -> &'static str {
        match self {
            SbcCategory::Smooth => "smooth",
            SbcCategory::Erratic => "erratic",
            SbcCategory::Intermittent => "intermittent",
            SbcCategory::Lumpy => "lumpy",
        }
    }
}
...
pub fn classify_sbc(adi: f64, cv2: f64) -> SbcCategory {
    if adi < 1.32 {
        if cv2 < 0.49 {
            SbcCategory::Smooth
        } else {
...
pub fn calculate_sbc_metrics(serie: &[f64]) -> Option<(f64, f64)> {
    let total_days = serie.len() as f64;
    let non_zero_demands: Vec<f64> = serie.iter().copied().filter(|&v| v > 0.0).collect();
    let active_days = non_zero_demands.len();
    if active_days == 0 {
        return None;
    }

    let adi = total_days / (active_days as f64);

    let mean = non_zero_demands.iter().sum::<f64>() / (active_days as f64);
    if mean == 0.0 {
        return Some((adi, 0.0));
    }

    let var = if active_days < 2 {
        0.0
    } else {
        let sum_sq_diff: f64 = non_zero_demands.iter().map(|&x| (x - mean).powi(2)).sum();
        sum_sq_diff / ((active_days - 1) as f64)
    };

    let cv2 = var / mean.powi(2);

    Some((adi, cv2))
}

pub fn sba_simulation(serie_diaria: &[f64], alpha: f64) -> f64 {
    if serie_diaria.is_empty() {
        return 0.0;
    }

    let non_zero_demands: Vec<f64> = serie_diaria.iter().copied().filter(|&v| v > 0.0).collect();
    let active_days = non_zero_demands.len();
    if active_days == 0 {
        return 0.0;
    }
    ...
    let mean = non_zero_demands.iter().sum::<f64>() / (active_days as f64);
    let adi = (serie_diaria.len() as f64) / (active_days as f64);

    let mut z = mean;
    let mut p = adi;
    let mut q = 1.0;

    for &y in serie_diaria {
        if y > 0.0 {
...
pub fn compute_forecast(
    ...
    let sbc_metrics = calculate_sbc_metrics(serie_diaria);
    let category = match sbc_metrics {
        Some((adi, cv2)) => classify_sbc(adi, cv2),
        None => SbcCategory::Smooth,
    };

    let serie_w = winsorize_p95(serie_diaria);
    let sigma = stddev_sample(&serie_w);

    let mu = match category {
        SbcCategory::Smooth | SbcCategory::Erratic => ewma(&serie_w, 0.2),
        SbcCategory::Intermittent | SbcCategory::Lumpy => sba_simulation(serie_diaria, 0.15),
    };
    ...
    let razon = match category {
        SbcCategory::Smooth | SbcCategory::Erratic => {
            format!(
                "μ={:.2} u/día (EWMA winsorizada p95, category: {:?}, {} días con consumo de {}), \
                 σ={:.2}, L={:.0}d, T={:.0}d, Z={:.2}",
                mu,
                category,
                dias_con_consumo,
        ...
        SbcCategory::Intermittent | SbcCategory::Lumpy => {
            format!(
                "μ={:.2} u/día (SBA raw, category: {:?}, {} días con consumo de {}), \
                 σ={:.2}, L={:.0}d, T={:.0}d, Z={:.2}",
                mu,
                category,
                dias_con_consumo,
...
mod tests {
    ...
    #[test]
    fn test_calculate_sbc_metrics() {
        let mut serie_smooth = vec![0.0; 60];
        for i in 0..50 {
            serie_smooth[i] = 5.0;
        }
        let metrics = calculate_sbc_metrics(&serie_smooth).unwrap();
        assert!(approx(metrics.0, 1.20, 0.0001));
        assert!(approx(metrics.1, 0.0, 0.0001));

        let mut serie_lumpy = vec![0.0; 60];
        // 10 demands of 10.0 and 5 demands of 20.0
        for i in 0..10 {
            serie_lumpy[i] = 10.0;
        }
        for i in 10..15 {
        ...
        let metrics_lumpy = calculate_sbc_metrics(&serie_lumpy).unwrap();
        assert!(approx(metrics_lumpy.0, 4.00, 0.0001));
        assert!(approx(metrics_lumpy.1, 0.1339, 0.001));
    }

    #[test]
    fn test_sba_simulation() {
        let serie1 = vec![0.0, 10.0, 0.0, 0.0];
        let forecast1 = sba_simulation(&serie1, 0.15);
        // expected final level = 10.0, interval = 3.7
        // bias corrected = (1 - 0.15/2) * 10.0 / 3.7 = 0.925 * 10.0 / 3.7 = 2.5
        assert!(approx(forecast1, 2.5, 0.0001));

        let serie2 = vec![10.0, 0.0, 10.0, 0.0];
        let forecast2 = sba_simulation(&serie2, 0.15);
        // expected final level = 10.0, interval = 1.8725
        // bias corrected = 0.925 * 10.0 / 1.8725 = 4.93992
        assert!(approx(forecast2, 4.93992, 0.0001));
    }

    #[test]
    fn test_compute_forecast_routing() {
        let cfg = ForecastConfig::default();

        // 1. Smooth series: 60 days of 5.0
        let smooth_serie = vec![5.0; 60];
        let r_smooth = compute_forecast(&smooth_serie, 100.0, 0.0, 7, cfg);
        assert_eq!(r_smooth.confianza, Confianza::Alta);
        assert!(approx(r_smooth.mu, 5.0, 0.01));
        assert!(r_smooth.razon.to_lowercase().contains("smooth"));

        // 2. Intermittent series: 60 days, last 15 with consumption of 10.0 (45 zeros first)
        let mut intermittent_serie = vec![0.0; 60];
        for i in 45..60 {
            intermittent_serie[i] = 10.0;
        }
        let r_intermittent = compute_forecast(&intermittent_serie, 100.0, 0.0, 7, cfg);
        assert_eq!(r_intermittent.confianza, Confianza::Media);
        // expected final level = 10.0, final interval = 1.95576, expected SBA = 4.7296
        assert!(approx(r_intermittent.mu, 4.7296, 0.01));
        assert!(r_intermittent.razon.to_lowercase().contains("intermittent"));
    }

    #[test]
    fn test_sbc_quadrants_coverage() {
        let cfg = ForecastConfig::default();

        // 1. Smooth
        let smooth_serie = vec![5.0; 60];
        let r_smooth = compute_forecast(&smooth_serie, 100.0, 0.0, 7, cfg);
        assert!(r_smooth.razon.contains("Smooth"));

        // 2. Erratic
        let mut erratic_serie = vec![2.0; 60];
        for i in (0..60).step_by(2) {
            erratic_serie[i] = 18.0;
        }
        let r_erratic = compute_forecast(&erratic_serie, 100.0, 0.0, 7, cfg);
        assert!(r_erratic.razon.contains("Erratic"));
        ...
        let r_intermittent = compute_forecast(&intermittent_serie, 100.0, 0.0, 7, cfg);
        assert!(r_intermittent.razon.contains("Intermittent"));

        // 4. Lumpy
        let mut lumpy_serie = vec![0.0; 60];
        for i in 45..55 {
            lumpy_serie[i] = 5.0;
        }
        for i in 55..60 {
        ...
        let r_lumpy = compute_forecast(&lumpy_serie, 100.0, 0.0, 7, cfg);
        assert!(r_lumpy.razon.contains("Lumpy"));
    }

    #[test]
    fn test_sbc_classification() {
        assert_eq!(classify_sbc(1.20, 0.10), SbcCategory::Smooth);
        assert_eq!(classify_sbc(1.20, 0.60), SbcCategory::Erratic);
        assert_eq!(classify_sbc(4.00, 0.30), SbcCategory::Intermittent);
        assert_eq!(classify_sbc(4.00, 0.80), SbcCategory::Lumpy);
    }
}
