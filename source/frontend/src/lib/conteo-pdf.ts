...
function drawFooter(doc: jsPDF, nombreLaboratorio: string) {
  ...
  for (let page = 1; page <= pages; page++) {
    ...
    doc.text(
      "Documento generado electronicamente por el Sistema de Inventario.",
      12,
      H - 7,
    );
    doc.text(
      `${nombreLaboratorio} - Pagina ${page} de ${pages}`,
      W - 12,
      H - 7,
      { align: "right" },
    );
...
function drawSesionMeta(
  ...
  doc.text(
    `Confirmada: ${sesion.confirmed_at ? formatDateTime(sesion.confirmed_at) : "Pendiente"}`,
    12,
    y + 16,
  );
  ...
  doc.setTextColor(...C.muted);
  doc.text(
    `Total positivo: ${formatCantidad(r.positivos, "unidad", "unidades")}   Total negativo: ${formatCantidad(r.negativos, "unidad", "unidades")}`,
    130,
    y + 19,
  );
...
export async function exportarConteoSesionPDF(
  options: ConteoPdfOptions,
): Promise<void> {
  const doc = new jsPDF({
...
export async function exportarConteoGlobalDiaPDF(
  options: ConteoGlobalPdfOptions,
): Promise<void> {
  const sesiones = (await fetchSesionesConfirmadas())
    .filter(
      (s) => s.confirmed_at && sameLocalDay(s.confirmed_at, options.fecha),
    )
  ...
  const detalles = await Promise.all(
    sesiones.map((s) =>
      api.get<ConteoDetalleConNota>(`/conteo/${s.id}`).then((r) => r.data),
    ),
  );
  ...
  doc.setTextColor(...C.muted);
  doc.text(
    "Detalle por area en paginas siguientes.",
    W - 12,
    (doc.lastAutoTable?.finalY ?? 120) + 8,
    { align: "right" },
  );
