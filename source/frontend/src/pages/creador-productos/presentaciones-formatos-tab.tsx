...
export default function PresentacionesFormatosTab() {
  ...
  const { data: formatos = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      api.get<PresFormatoRow[]>("/presentacion-formatos").then((r) => r.data),
  });

  const createMut = useMutation({
    mutationFn: (payload: PresFormato) =>
      api.post("/presentacion-formatos", payload),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="form-control">
            ...
            <input
              ...
              onChange={(e) =>
                setNuevo({ ...nuevo, nombre_plural: e.target.value })
              }
            />
          </div>
        </div>
        <button
          type="submit"
          className="btn btn-primary btn-sm gap-1.5 w-full mt-1"
