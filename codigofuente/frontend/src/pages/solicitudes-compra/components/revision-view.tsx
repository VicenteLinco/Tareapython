...
const unitLabel = (qty: number, singular: string, plural?: string | null) =>
  formatCantidad(qty, singular, plural ?? undefined)
    .replace(/^[\d.,\s]+/, "")
    .trim();
...
export function RevisionView({
  ...
  const [cantidadesPendientes, setCantidadesPendientes] = useState<
    Record<string, string>
  >({});

  const aceptadosIds = new Set(itemsEnPedido.map((i) => i.producto_id));

  const pendientesRaw = recomendaciones.filter(
    (r) => !aceptadosIds.has(r.producto_id) && !descartados.has(r.producto_id),
  );
  const descartadosList = recomendaciones.filter((r) =>
    descartados.has(r.producto_id),
  );
  const aceptados = recomendaciones.filter((r) =>
    aceptadosIds.has(r.producto_id),
  );
  ...
  const criticosPendientes = pendientes.filter(
    (r) =>
      (r.nivel_urgencia === "critica" || r.nivel_urgencia === "critico") &&
      r.confianza !== "baja",
  );

  const getCantidadInput = (r: ItemRecomendado): string => {
    if (r.producto_id in cantidadesPendientes)
      return cantidadesPendientes[r.producto_id];
    return r.confianza !== "baja"
      ? String(toDecimal(r.cantidad_sugerida_base).ceil().toNumber())
      : "";
  };
  ...
  const handleAgregarAlPedido = (r: ItemRecomendado) => {
    ...
    if (val > 0) {
      ...
      setCantidadesPendientes((prev) => {
        const next = { ...prev };
        delete next[r.producto_id];
        return next;
      });
    }
  };
  ...
  const confianzaInfo = (c: string) => {
    if (c === "alta")
      return {
        ...
        tooltip:
          "Predicción basada en historial sólido (≥30 días). La cantidad sugerida es confiable.",
      };
    if (c === "media")
      return {
        label: "Media",
        ...
        tooltip:
          "Historial parcial (14–29 días). Cantidad orientativa, se recomienda revisar.",
      };
    return {
      label: "Baja",
      color: "text-base-content/50 bg-base-200",
      tooltip:
        "Historial insuficiente (<14 días). No se genera cantidad automática — debes ingresar la cantidad.",
    };
  };
  ...
  const renderFila = (
    r: ItemRecomendado,
    estado: "pendiente" | "aceptado" | "descartado",
  ) => {
    const isCritica =
      r.nivel_urgencia === "critica" || r.nivel_urgencia === "critico";
    ...
    const itemEnPedido = itemsEnPedido.find(
      (i) => i.producto_id === r.producto_id,
    );
        ...
        <div className="flex-1 min-w-0 space-y-1.5">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-base-content/50">
            <span title="Stock actual / mínimo">
              ...
              / {stockMin}{" "}
              {unitLabel(stockMin, r.unidad_base, r.unidad_base_plural)}
            </span>
            {consumoDiario > 0 && (
              <span>
                Cons/día:{" "}
              ...
              <div className="flex items-center gap-1.5">
                ...
                >
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && cantidadInputValida)
                        handleAgregarAlPedido(r);
                    }}
                  />
                  <button
                    className="btn btn-ghost btn-xs p-0 h-5 w-5 min-h-0 rounded-lg"
                  ...
                  <span className="text-[9px] text-base-content/40 ml-0.5">
                    {unitLabel(
                      toNum(cantidadInput),
                      r.unidad_base,
                      r.unidad_base_plural,
                    )}
                  </span>
                </div>
                <button
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              <div className="flex items-center gap-1 bg-success/10 rounded-xl px-1.5 py-0.5">
                <button
                  className="btn btn-ghost btn-xs p-0 h-5 w-5 min-h-0 rounded-lg"
                  onClick={() => {
                    if (cantidadAceptada > 1)
                      onUpdateQty(r.producto_id, cantidadAceptada - 1);
                  }}
                >
                  <Minus className="h-3 w-3" />
                </button>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {criticosPendientes.length > 0 && (
            ...
            >
              ...
              Agregar {criticosPendientes.length} crítico
              {criticosPendientes.length !== 1 ? "s" : ""}
            </button>
          )}
