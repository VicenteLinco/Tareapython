...
export function StockItemCard({ item, view, isSelected, onClick }: StockItemCardProps) {
  const unitLabel = formatCantidad(item.stock_total ?? 0, item.unidad, item.unidad_plural ?? undefined)
    .replace(/^[\d.,\s]+/, '')
    .trim()
      ...
      >
        ...
        <td className="text-center">
          <div className="flex flex-col items-center">
            ...
            <span className="text-[9px] opacity-40 uppercase font-bold mt-1">{unitLabel}</span>
          </div>
        </td>
        <td>
        ...
        <td className="pr-6">
          <ChevronRight className={cn("w-4 h-4 transition-all opacity-0 group-hover:opacity-100", isSelected ? "translate-x-1 opacity-100 text-primary" : "opacity-20")} />
        </td>
      </tr>
    )
  }
