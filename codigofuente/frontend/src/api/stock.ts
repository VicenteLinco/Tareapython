...
export async function listarStock(
  ...
  const apiParams = params
    ? {
        ...params,
        area_ids: Array.isArray(params.area_ids)
          ? params.area_ids.join(",")
          : params.area_ids,
      }
    : undefined;
...
export async function stockPorArea(
  areaId: number,
  params?: { page?: number; per_page?: number; q?: string },
): Promise<StockAreaResponse> {
  const { data } = await api.get<StockAreaResponse>(`/stock/area/${areaId}`, {
    params,
  });
  return data;
}
...
export async function obtenerAlertas(
  ...
  const apiParams = params
    ...
    : undefined;
  const { data } = await api.get<PaginatedResponse<Alerta>>("/stock/alertas", {
    params: apiParams,
  });
  return data;
}
...
export async function crearConsumoBatch(
  payload: ConsumoBatchRequest,
): Promise<ConsumoResponse> {
...
export async function crearDescarte(
  payload: DescarteRequest,
): Promise<DescarteResponse> {
  const { data } = await api.post<DescarteResponse>("/descartes", payload);
  return data;
}

/** GET /descartes — Historial de descartes paginado */
export async function historicoDescartes(
  params?: DescartesHistorialQuery,
): Promise<PaginatedResponse<DescarteSession>> {
  const { data } = await api.get<PaginatedResponse<DescarteSession>>(
    "/descartes",
    { params },
  );
  return data;
