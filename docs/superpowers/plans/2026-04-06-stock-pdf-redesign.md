# Stock PDF Redesign — Minimal Pro

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Reemplazar el generador de PDF de inventario (`stock-pdf.ts`) con un diseño "Minimal Pro" — tipografía bold oversized, KPI strip, barras de nivel de stock, y paleta monocromática con acentos funcionales.

**Architecture:** Reescritura completa de `stock-pdf.ts`. Un solo archivo, mismas interfaces públicas (`exportarStockPDF`, `PdfOptions`). Render en dos fases: (1) dibujar todas las páginas, (2) post-pass para escribir "Página N de T" en todos los footers.

**Tech Stack:** jsPDF 4.x + jspdf-autotable 5.x. Sin cambios de dependencias.
...
| Archivo | Acción |
|---------|--------|
| `frontend/src/lib/stock-pdf.ts` | Reescritura completa |

Nada más cambia. Interfaces públicas idénticas, solo el rendering cambia.

---

## Task 1 — Paleta, helpers y tipos internos

**Files:**
- Modify: `frontend/src/lib/stock-pdf.ts` (líneas 1–77)

Reemplazar la paleta de colores, agregar helper `badgeDate`, y actualizar `getAlerta`.

- [ ] **Paso 1: Reemplazar la sección de paleta y helpers al tope del archivo**

Reemplazar desde la línea 1 hasta el cierre de `getAlerta` (actualmente línea 77) con:
...
```bash
cd frontend && npx tsc --noEmit 2>&1 | grep stock-pdf
```
Esperado: sin output (sin errores).

---

## Task 2 — `fetchStockForArea`, `drawHeader` y `drawFooterFinal`

**Files:**
- Modify: `frontend/src/lib/stock-pdf.ts` (continúa después del Task 1)

- [ ] **Paso 1: Agregar `fetchStockForArea` (sin cambios respecto al actual)**
...
- [ ] **Paso 2: Agregar `drawHeader` (nuevo diseño)**

Constantes de layout a agregar antes de la función:
...
- [ ] **Paso 3: Agregar `drawFooterFinal` (post-pass con total de páginas)**
...
function drawFooterFinal(
  ...
  // Cubrir footer previo (si existe) con rect blanco
...
## Task 3 — Página de Resumen Ejecutivo

**Files:**
- Modify: `frontend/src/lib/stock-pdf.ts`

- [ ] **Paso 1: Agregar helper `drawResumen`**
...
function drawResumen(
  ...
  doc.setFontSize(20)
  doc.setTextColor(...C.black)
  // Dos líneas: "Resumen" y "Ejecutivo"
...
## Task 4 — Páginas de tabla por área

**Files:**
- Modify: `frontend/src/lib/stock-pdf.ts`

- [ ] **Paso 1: Agregar helper `drawAreaPage`**

Esta función dibuja la tabla de una área usando `autoTable`. El header y footer los maneja `didDrawPage` internamente.
  ...
  autoTable(doc, {
    ...
    didDrawPage: (data: any) => {
      ...
      if (isFirstPage) {
        ...
        let statsStr = `${items.length} ${items.length === 1 ? 'producto' : 'productos'}`
...
## Task 5 — Función principal `exportarStockPDF` y post-pass de footers

**Files:**
- Modify: `frontend/src/lib/stock-pdf.ts`

- [ ] **Paso 1: Reemplazar la función `exportarStockPDF` completa**
...
- [ ] **Paso 2: Verificar TypeScript sin errores**
...
- [ ] **Paso 3: Verificar que el archivo compila completo**
...
git add frontend/src/lib/stock-pdf.ts
git commit -m "feat(pdf): redesign stock report — Minimal Pro style

- Nuevo header: logo circular, badge de fecha negro, meta del usuario
- KPI strip con 4 tarjetas unidas (productos, bajo mínimo, por vencer, vencidos)
- Columnas de alertas con dot + borde de color
- Tabla 7 columnas con barras de nivel de stock por fila
- Filas coloreadas por estado (rojo suave / ámbar suave)
- Mini-stats del área (bajoCuenta, vencerCuenta) en encabezado
- Post-pass para footer 'Página N de T'
- Nombre de archivo: stock-inventario-YYYY-MM-DD.pdf"
```

- [ ] **Paso 5: Probar en el navegador**

1. Abrir `http://localhost:5173/stock`
2. Click en **Exportar**
3. Seleccionar áreas con stock → **Generar PDF**
4. Verificar que el PDF se descarga con el nombre `stock-inventario-YYYY-MM-DD.pdf`
5. Abrir el PDF y confirmar:
   - Header con logo/badge en todas las páginas
   - Resumen ejecutivo con KPI strip y alertas
   - Tablas con barras de nivel de stock
   - Footer con "Página N de T"

---

## Notas de implementación

- **`doc.circle()`** — jsPDF v4 soporta `doc.circle(x, y, r, style)` donde x,y es el **centro**.
- **Barras de nivel**: `ratio = clamp(stock / (minimo * 3), 0, 1)`. Sin mínimo configurado → 0.7.
- **Post-pass de footers**: `doc.setPage(p)` permite volver a cualquier página ya renderizada. `drawFooterFinal` dibuja un rect blanco encima del área de footer antes de escribir el texto nuevo.
- **Fuentes disponibles**: helvetica, courier (ambas integradas en jsPDF sin necesidad de cargar nada extra).
