...
export function GlobalSearch({ open, onClose }: GlobalSearchProps) {
  ...
  const { data: stockData } = useQuery({
    ...
    queryFn: () =>
      api.get<PaginatedResponse<StockItem>>('/stock', { params: { q: debouncedQuery, per_page: 6 } }).then(r => r.data),
    enabled,
    staleTime: 30_000,
  })
  ...
  const { data: recData } = useQuery({
    ...
    queryFn: () =>
      api.get<{ data: RecepcionListItem[] }>('/recepciones', { params: { q: debouncedQuery, per_page: 4 } }).then(r => r.data),
    enabled,
    staleTime: 30_000,
  })

  const navAndClose = useCallback((path: string) => {
    navigate(path)
    onClose()
  }, [navigate, onClose])
  ...
  const acciones: ResultItem[] = ACCIONES_RAPIDAS
    .filter(a =>
      query.length < 2 ||
      a.title.toLowerCase().includes(lowerQuery) ||
      a.keywords.some(k => k.includes(lowerQuery))
    )
    .map(a => ({ id: a.id, type: 'accion' as const, title: a.title, subtitle: a.subtitle, action: () => navAndClose(a.path) }))
  ...
  const renderGroup = (label: string, items: ResultItem[]) => {
    ...
    return (
      <div key={label}>
        <div className="px-4 pt-3 pb-1 text-[10px] font-bold uppercase tracking-widest opacity-30">{label}</div>
        {items.map(item => {
          ...
          return (
            ...
            >
              <div className={cn('shrink-0 opacity-40 transition-colors', isActive && 'opacity-100 text-primary')}>
                {typeIcon(item.type)}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium truncate">{item.title}</div>
                {item.subtitle && <div className="text-[11px] opacity-40 truncate">{item.subtitle}</div>}
              </div>
              {item.badge && (
                <span className="text-[10px] font-mono opacity-40 shrink-0">{item.badge}</span>
              )}
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-base-200">
          ...
          />
          {query ? (
            <button onClick={() => setQuery('')} className="p-1 hover:bg-base-200 rounded-lg transition-colors">
        ...
        <div className="overflow-y-auto max-h-[55vh]" role="listbox">
          {results.length === 0 && query.length >= 2 ? (
            <div className="py-10 text-center text-sm opacity-40">Sin resultados para "{query}"</div>
          ) : (
            <>
