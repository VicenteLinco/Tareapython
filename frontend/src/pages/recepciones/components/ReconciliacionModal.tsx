...
export function ReconciliacionModal({
  ...
  detalles.forEach((d) => {
    const total = d.lotes.reduce(
      (s, l) => s + l.cantidad_presentacion * d.factor_conversion,
      0,
    );
  ...
  const filasExtra = Object.entries(recibidoMap)
    ...
    .map(([productoId, recibido]) => {
      ...
      return {
        ...
        producto_nombre:
          recibidoNombreMap[productoId] ?? "Producto no solicitado",
        cantidad_base: 0,
  ...
  const tieneFaltantes = filas.some(
    (f) => f.estado === "faltante" || f.estado === "no_recibido",
  );
  ...
  const formatFilaQty = (total: number, item: (typeof filas)[0]) => {
    ...
    const uBasePlural =
      item.unidad_plural || (uBase === "unidad" ? "unidades" : uBase);
    ...
    if (item.presentacion_id && factor > 1) {
      const presText = formatStockHumano(
        total,
        factor,
        uBase,
        uBasePlural,
      ...
      <div className="bg-base-100 rounded-3xl shadow-2xl w-full max-w-2xl border border-base-200 overflow-hidden">
        <div className="px-6 py-5 border-b border-base-200">
          <h2 className="font-bold text-base">
            Comparar con solicitud vinculada
          </h2>
          <p className="text-xs opacity-50 mt-0.5">
            Revisa faltantes, sobrantes e items extra antes de confirmar la
            recepcion.
          </p>
        </div>
            ...
            >
              ...
              <div className="text-right text-xs tabular-nums flex flex-col items-end gap-0.5">
                <span className="opacity-50">
                  Pedido: {formatFilaQty(f.cantidad_base, f)}
                </span>
                <span className="font-bold">
                  Llegó: {formatFilaQty(f.recibido, f)}
                </span>
              </div>
          ...
          <div className="px-6 pb-2">
            ...
            {tieneFaltantes && (
              <p className="text-xs opacity-60 mt-1">
                Si faltan items, la solicitud quedara como recepcion parcial y
                no se cerrara.
              </p>
            )}
          </div>
