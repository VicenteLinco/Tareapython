...
export function AsignarCodigoModal({
  ...
  useEffect(() => {
    if (unidades && unidades.length > 0 && !unidadBaseId) {
      ...
      const u = unidades.find(x => x.nombre.toLowerCase().includes('unidad') || x.nombre.toLowerCase().includes('u.')) || unidades[0]
  ...
  const handleSubmitCreate = (e: React.FormEvent) => {
    e.preventDefault()
    if (!nombre.trim()) { notify.error('El nombre del producto es requerido'); return }
    if (!unidadBaseId) { notify.error('Selecciona una unidad base'); return }
    if (!areaId) { notify.error('Selecciona un área de destino'); return }
  ...
  return (
    <Dialog open onClose={onClose} title="Código desconocido escaneado" className="max-w-2xl">
      <div className="space-y-4">
        ...
        <div className="flex flex-col gap-1.5 p-3.5 bg-base-200 border border-base-300 rounded-2xl">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-bold uppercase tracking-widest text-base-content/40">Código escaneado</span>
            {isGs1 ? (
          ...
          <span className="font-mono text-sm font-bold text-primary break-all">{codigo}</span>
          ...
          {isGs1 && parsedCode && (
            <div className="mt-2.5 pt-2.5 border-t border-base-300 grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
              <div>
                ...
                <span className="font-mono font-semibold">{parsedCode.gtin}</span>
              </div>
              {parsedCode.ref && (
                <div>
                  <span className="opacity-50 font-medium">REF:</span>{' '}
                  <span className="font-mono font-semibold">{parsedCode.ref}</span>
                </div>
              )}
              {parsedCode.lote && (
                <div>
                  <span className="opacity-50 font-medium">Lote detectado:</span>{' '}
                  <span className="font-mono font-semibold badge badge-warning badge-outline text-[10px] h-4 py-0 px-1">{parsedCode.lote}</span>
                </div>
              )}
              {parsedCode.vencimiento && (
                <div>
                  <span className="opacity-50 font-medium">Vencimiento:</span>{' '}
                  <span className="font-mono font-semibold text-warning">{parsedCode.vencimiento}</span>
                </div>
              )}
            </div>
          )}
        ...
        {existingProductWithRef ? (
          <div className="p-4 bg-primary/10 border border-primary/25 rounded-2xl space-y-3">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                <h4 className="font-bold text-sm text-primary">¡Insumo existente detectado por REF!</h4>
                <p className="text-xs text-base-content/70 mt-0.5">
                  El código de catálogo <strong className="font-mono">{refValueFromScan}</strong> ya pertenece a un producto en tu sistema. Puedes vincular esta presentación (nuevo código de barras) y agregarlo directamente:
                </p>
                <p className="text-sm font-bold mt-2 text-base-content">{existingProductWithRef.nombre}</p>
                {existingProductWithRef.codigo_interno && (
                  <span className="text-[10px] font-mono opacity-50">Código interno: #{existingProductWithRef.codigo_interno}</span>
                )}
              </div>
            </div>
            ...
            <div className="p-3 bg-base-100 rounded-xl border border-base-200 grid grid-cols-2 gap-3">
              <div className="form-control">
                <label className="label py-0">
                  <span className="label-text text-[10px] font-bold text-base-content/50 uppercase">Lote</span>
                </label>
                <input
                  type="text"
              ...
              <div className="form-control">
                <label className="label py-0">
                  <span className="label-text text-[10px] font-bold text-base-content/50 uppercase">Vencimiento</span>
                </label>
                <input
                  type="date"
                  className="input input-bordered input-sm h-8 w-full bg-base-100 font-mono text-xs"
            ...
            <div className="flex justify-end gap-2 pt-1.5">
              <button
                ...
                onClick={() => {
                  ...
                  refValueFromScan && notify.info('Ignorando coincidencia automática de REF.');
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="form-control col-span-2">
                    <label className="label py-0.5">
                      <span className="label-text text-xs font-semibold">Nombre del Producto</span>
                  ...
                  <div className="form-control">
                    <label className="label py-0.5">
                      <span className="label-text text-xs font-semibold">Categoría</span>
                  ...
                  <div className="form-control">
                    <label className="label py-0.5">
                      <span className="label-text text-xs font-semibold">Unidad Base</span>
                      <span className="label-text-alt text-error text-[10px]">requerido</span>
                    </label>
                    <select
                      className="select select-bordered select-sm h-9 bg-base-100 w-full"
                  ...
                  <div className="form-control">
                    <label className="label py-0.5">
                      <span className="label-text text-xs font-semibold">Área de Destino</span>
                      <span className="label-text-alt text-error text-[10px]">requerido</span>
                    </label>
                    <select
                      className="select select-bordered select-sm h-9 bg-base-100 w-full"
                  ...
                  <div className="form-control">
                    <label className="label py-0.5">
                      <span className="label-text text-xs font-semibold">Código REF (Catálogo)</span>
                      <span className="label-text-alt text-base-content/40 text-[10px]">{isGs1 ? 'autodetectado' : 'opcional'}</span>
                    </label>
                    <input
                      type="text"
                  ...
                  <div className="form-control">
                    <label className="label py-0.5">
                      <span className="label-text text-xs font-semibold">GTIN / Código</span>
                      <span className="label-text-alt text-base-content/40 text-[10px]">{isGs1 ? 'autodetectado' : 'autocompletado'}</span>
                    </label>
                    <input
                      type="text"
                      className="input input-bordered input-sm h-9 w-full bg-base-100 disabled:opacity-75 font-mono"
                  ...
                  <div className="col-span-2 mt-2 p-3 bg-primary/5 rounded-xl border border-primary/10 space-y-2">
                    <h4 className="text-xs font-bold text-primary flex items-center gap-1.5">
                      <Info className="h-3.5 w-3.5" /> Datos de recepción del lote escaneado
                    </h4>
              ...
              <form onSubmit={handleSubmitLink} className="space-y-4">
                <div className="form-control">
                  ...
                  <p className="text-[10px] text-base-content/40 mt-1">
                    Al vincular, el código quedará registrado en este producto y podrás escanearlo directamente en el futuro.
                  </p>
                </div>
