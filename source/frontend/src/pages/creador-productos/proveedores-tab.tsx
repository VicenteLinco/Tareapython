...
export default function ProveedoresTab() {
  ...
  const [reactivateTarget, setReactivateTarget] = useState<Proveedor | null>(
    null,
  );
  ...
  function openEdit(p: Proveedor) {
    ...
    setForm({
      ...
      icono: p.icono ?? "",
      dias_despacho_aereo:
        p.dias_despacho_aereo != null ? String(p.dias_despacho_aereo) : "",
      dias_despacho_tierra:
        p.dias_despacho_tierra != null ? String(p.dias_despacho_tierra) : "",
    });
    setFormMode("editar");
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const clean: CreateProveedor = {
      ...
      dias_despacho_aereo: form.dias_despacho_aereo
        ? Number(form.dias_despacho_aereo)
        : null,
      dias_despacho_tierra: form.dias_despacho_tierra
        ? Number(form.dias_despacho_tierra)
        : null,
    };
    if (selectedItem) {
      updateMut.mutate({
        id: selectedItem.id,
        data: { ...clean, version: selectedItem.version },
      });
    } else {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="flex gap-3">
        ...
        <div className="form-control flex-1">
          <label className="label py-1">
            <span className="label-text text-xs font-semibold">Nombre *</span>
          </label>
          <input
            type="text"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <div className="form-control">
          <label className="label py-1">
            <span className="label-text text-xs font-semibold">Teléfono</span>
          </label>
          <input
            type="tel"
            className="input input-bordered input-sm"
        ...
        <div className="form-control">
          <label className="label py-1">
            <span className="label-text text-xs font-semibold">Ejecutivo</span>
          </label>
          <input
            type="text"
            className="input input-bordered input-sm"
  ...
  const columns = [
    {
      ...
      render: (item: Proveedor) => (
        <div
          className={`flex items-center gap-3 w-full overflow-hidden ${!item.activa ? "opacity-50" : ""}`}
          title={item.nombre}
        >
          <ProveedorIcon
            proveedor={item}
            className="h-8 w-8 shrink-0 bg-base-200 rounded p-0.5"
          />
    ...
    },
    {
      ...
      render: (item: Proveedor) => (
        <div
          className={`flex flex-col min-w-0 w-full overflow-hidden ${!item.activa ? "opacity-50" : ""}`}
        >
    ...
    },
    {
      ...
      render: (item: Proveedor) => (
        <div
          className={`flex gap-2 text-[10px] opacity-60 ${!item.activa ? "opacity-30" : ""}`}
        >
  ...
  const groupedSearchItems = (() => {
    const result: (
      ...
      | { type: "item"; item: (typeof proveedores)[number]; idx: number }
    )[] = [];
    let lastL = "";
  ...
  return (
    <div className="space-y-4">
        ...
        {formMode !== "idle" && isDesktop && (
          <div className="hidden lg:flex flex-col min-w-0 lg:flex-[2] lg:sticky lg:top-24">
            <div className="rounded-xl border border-base-300 bg-base-100 flex flex-col max-h-[calc(100vh-120px)]">
              ...
              <div className="flex-1 overflow-y-auto px-5 pb-5">{formJsx}</div>
            </div>
          </div>
        )}
      </div>
      ...
      <Dialog
        ...
        onClose={closeForm}
        title={formMode === "crear" ? "Nuevo Proveedor" : "Editar Proveedor"}
        className="max-w-md"
      >
        {formJsx}
      </Dialog>
