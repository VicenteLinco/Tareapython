...
function buildScannedPresentacion(data: ScannedPresentacionData) {
  return {
    ...
    nombre_plural: data.presentacion_nombre
      ? `${data.presentacion_nombre}s`
      : "",
    factor_conversion: "1",
    codigo_barras: null,
    gtin: null,
...
function buildDetalleLineFromScan(
  ...
  const precio = data.precio_unidad
    ? mulDecimal(data.precio_unidad, data.factor_conversion || 1)
        .toDecimalPlaces(0)
        .toString()
    : "";
  return {
    ...
    presentacion_nombre: data.presentacion_nombre || "",
    presentacion_nombre_plural: data.presentacion_nombre
      ? data.presentacion_nombre + "s"
      : "",
    cantidad_solicitada: null,
    factor_conversion: data.factor_conversion
      ? toNum(data.factor_conversion)
      : 1,
...
export function useRecepcionItems({
  ...
  const [lotesConfirmados, setLotesConfirmados] = useState<
    LoteParaEtiqueta[] | null
  >(null);
  const printModal = useDialogState();

  // Reconciliación post-recepción
  const [solicitudItemsRef, setSolicitudItemsRef] = useState<
    SolicitudItemSimple[]
  >([]);
  const reconciliacionModal = useDialogState();
  const [pendingConfirmarPayload, setPendingConfirmarPayload] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const validarProveedorEscaneado = useCallback(
    (
      productoNombre: string,
      productoProveedorId: number | null | undefined,
    ) => {
      if (!proveedorId || productoProveedorId === proveedorId) return true;
      if (productoProveedorId) {
        const nombreProv =
          proveedores?.find((p) => p.id === productoProveedorId)?.nombre ??
          "otro proveedor";
  ...
  const fetchSupplierPresentaciones = useCallback(
    async (
      ppId: number,
    ): Promise<Array<{
      id: number;
    ...
    async (
      ...
      if (proveedorId && prod.proveedor_id !== proveedorId) {
        if (prod.proveedor_id) {
          const nombreProv =
            proveedores?.find((p) => p.id === prod.proveedor_id)?.nombre ??
            "otro proveedor";
      ...
      try {
        ...
        const pres =
          (defaultPresId
            ? activePresentaciones.find(
                (p: { id: number }) => p.id === defaultPresId,
              )
            : null) ??
          activePresentaciones[0] ??
          null;
        ...
        const line: DetalleLineUI = {
          ...
          precio_unitario: full.precio_unidad
            ? mulDecimal(full.precio_unidad, pres?.factor_conversion || 1)
                .toDecimalPlaces(0)
                .toString()
            : "",
  ...
  const addProductoConLote = useCallback(
    async (
  ...
  const handleSearch = useCallback(
    async (valor: string) => {
      ...
      try {
        ...
        if (!validarProveedorEscaneado(data.producto_nombre, data.proveedor_id))
          return;
        ...
        if (data.tipo === "lote") {
          ...
          const existingDetalle = detalles.find(
            (d) => String(d.producto_id) === String(data.producto_id),
          );
          if (existingDetalle) {
            setDetalles((prev) =>
              prev.map((d) =>
            ...
            notify.success(
              `Lote ${data.numero_lote} añadido a ${data.producto_nombre}`,
            );
            return;
          }
          ...
          const prod = productos?.find(
            (p) =>
              ...
              String(p.id) === String(data.producto_id),
          );
      ...
      try {
        ...
        if (data.tipo === "gs1") {
          ...
          if (outcome.kind === "increment") {
            ...
            notify.success(
              `+1 ${data.producto_nombre} (lote ${data.gs1?.numero_lote ?? ""})`,
            );
            return;
          }
          if (outcome.kind === "append-lote") {
            ...
            navigator.vibrate?.(50);
            notify.success(
              `Lote ${outcome.numeroLote} añadido a ${data.producto_nombre}`,
            );
            return;
          }
          if (outcome.kind === "new-line") {
            const line = buildDetalleLineFromScan(
              data,
              outcome.numeroLote,
              outcome.fechaVencimiento,
            );
            setDetalles((prev) => [line, ...prev]);
            setScanCount((prev) => prev + 1);
            navigator.vibrate?.(50);
  ...
  const handleConfirmLote = useCallback(
    async (loteData: {
      numero_lote: string;
      fecha_vencimiento: string;
      cantidad: number;
      ...
      try {
        ...
        const pres =
          (pendingScan.presentacionId
            ? presentaciones.find(
                (p: { id: number }) => p.id === pendingScan.presentacionId,
              )
            : null) ??
          presentaciones[0] ??
          null;
        ...
        const line: DetalleLineUI = {
          ...
          area_destino_nombre:
            catalogoArea?.nombre ?? pendingScan.areaNombre ?? "",
          presentaciones,
          precio_unitario: full.precio_unidad
            ? mulDecimal(full.precio_unidad, pres?.factor_conversion || 1)
                .toDecimalPlaces(0)
  ...
  const handleChange = useCallback(
    (id: string, patch: Partial<Omit<DetalleLineUI, "lotes">>) => {
      setDetalles((prev) =>
        prev.map((d) => {
          if (d.id !== id) return d;
          ...
          if (patch.factor_conversion !== undefined && d.precio_base) {
            ...
            const newPrice = mulDecimal(d.precio_base, newFactor)
              .toDecimalPlaces(0)
              .toString();
            extraPatch = {
  ...
  const confirmarMutation = useMutation({
    mutationFn: async (payload: object) => {
      ...
      if (fotoGuia) {
        await api.put(`/recepciones/${res.data.id}/foto`, {
          data_url: fotoGuia,
        });
      }
      return res.data;
      ...
      const paraImprimir: LoteParaEtiqueta[] = lotes
        .map<LoteParaEtiqueta | null>((l) => {
          for (const d of detalles) {
            const lote = d.lotes.find(
              (lo) =>
                lo.codigo_lote === l.numero_lote &&
                d.producto_nombre === l.producto_nombre,
            );
  ...
  const handleConfirmar = useCallback(() => {
    ...
    if (!guiaDespacho.trim() && !guiaProvisoria) {
      notify.error(
        "Ingresa el número de guía de despacho o marca como provisoria",
      );
      return;
    }
    ...
    if (decision === "rechazada") {
      ...
      const motivos = [
        ...motivosSeleccionados.map(
          (id) => MOTIVOS_RECHAZO.find((m) => m.id === id)?.label ?? id,
        ),
    ...
    if (validos.length === 0) {
      notify.error(
        "Completa los datos requeridos (área destino, y lote/vencimiento si corresponde) en al menos un producto",
      );
      return;
    }
        ...
        d.lotes
          ...
          .map((l) => ({
            ...
            numero_lote:
              d.control_lote === "simple"
                ? undefined
                : l.codigo_lote || undefined,
            fecha_vencimiento:
              d.control_lote === "simple"
                ? undefined
                : l.fecha_vencimiento || undefined,
