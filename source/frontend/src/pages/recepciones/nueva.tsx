...
export default function NuevaRecepcionPage() {
  ...
  const guiaResumen =
    guiaDespacho || (guiaProvisoria ? "Provisorio" : "Sin guía");

  // ─── Vincular solicitud ───────────────────────────────────────────────────────

  const handleVincularSolicitud = async (
    id: string,
    numero: string,
    silent = false,
  ) => {
    try {
      ...
      const itemsProveedor = (
        (res.data.items ?? []) as SolicitudItemVinculada[]
      ).filter((it) => !proveedorId || it.proveedor_id === proveedorId);
      setSolicitudItemsRef(
        itemsProveedor.map((it) => {
          const factor = it.factor_conversion ? toNum(it.factor_conversion) : 1;
          const cantPres = it.cantidad_presentaciones
            ? toNum(it.cantidad_presentaciones)
            : null;
          ...
          return {
            ...
            cantidad_presentaciones:
              cantPres ?? (hasPres ? cantSugerida : null),
          };
        }),
      );
      for (const item of itemsProveedor) {
        try {
          const p = productos?.find(
            (x: Producto) => String(x.id) === String(item.producto_id),
          );
          if (p) {
    ...
    <div className="min-h-screen">
      ...
      <header className="sticky top-0 z-30 bg-base-100/95 backdrop-blur border-b border-base-200">
        <div className="flex items-center justify-between gap-3 px-4 py-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className={cn("badge badge-sm shrink-0", estadoInfo.cls)}>
              {estadoInfo.label}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 px-4 pb-2 text-xs text-base-content/60">
          ...
          <span>
            ...
            <strong className="text-base-content">{guiaResumen}</strong>
          </span>
          <span>
            {new Date(fechaRecepcion).toLocaleDateString(APP_LOCALE, {
              dateStyle: "short",
            })}
          </span>
          {detalles.length > 0 && (
      ...
      <div className="max-w-3xl mx-auto p-4 space-y-6">
        <section className="space-y-3">
          <SectionTitle
            n={1}
            title="Proveedor y guía"
            done={!!proveedorId && (!!guiaDespacho || guiaProvisoria)}
          />
          <ProveedorStep
            wizard={wizard}
