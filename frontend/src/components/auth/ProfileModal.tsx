  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (cleanPhone) {
      ...
      if (!phoneRegex.test(cleanPhone)) {
        setError('El número de WhatsApp no es válido (use dígitos, espacios o guiones, entre 8 y 20 caracteres)')
        return
      }
    } else {
      cleanPhone = null
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Editar mi perfil">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="form-control">
          <label className="label">
            <span className="label-text font-semibold text-xs">Número de WhatsApp (Opcional)</span>
          </label>
          <input
            type="text"
            className="input input-bordered input-sm w-full"
