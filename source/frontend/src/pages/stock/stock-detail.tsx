...
export function StockDetail({
  ...
  const [discardLote, setDiscardLote] = useState<{
    id: string;
    numeroLote: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    if (action === "discard" && lotes && lotes.length > 0) {
      ...
      if (oldestLote) {
        setDiscardLote({
          id: oldestLote.id,
          numeroLote: oldestLote.numero_lote,
        });
  ...
  const isLow =
    estado === "critico" || estado === "reponer" || estado === "agotado";
  const totalLotes =
    lotes?.reduce((s, l) => s + Math.round(l.stock_total ?? 0), 0) ??
    stockTotal;
      ...
      >
        ...
        <div className="flex items-baseline gap-2">
          ...
          <span className="text-sm opacity-40">
            {formatCantidad(stockTotal, item.unidad, item.unidad_plural ?? undefined)
              .replace(/^[\d.,\s]+/, "")
              .trim()}
          </span>
        </div>
          ...
          {item.dias_autonomia != null && (
            <div className="flex items-center justify-between px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                />
                {(item.dias_con_consumo ?? 0) > 0 &&
                  (item.dias_con_consumo ?? 0) < 14 && (
                    <span
      ...
      <div className="min-h-[200px]">
        {activeTab === "lotes" ? (
          <div className="space-y-2">
            {sortedLotes.length === 0 ? (
              ...
              sortedLotes.map((lote) => {
                ...
                const pct =
                  totalLotes > 0 ? Math.round((qty / totalLotes) * 100) : 0;
                ...
                const presNombrePlural =
                  presData?.presentacion_nombre_plural ?? null;
                const presEquivalente =
                  presData?.cantidad_presentaciones_equivalente ?? null;
                    ...
                    {canOperate && (
                      <div className="mt-3 flex justify-end gap-2">
                        <button
                          onClick={() =>
                            navigate(
                              `/consumos?search=${encodeURIComponent(item.producto_nombre)}`,
                            )
                          }
          ...
          <ProductTimeline
            ...
            areaId={areaId}
            unidad={item.unidad}
            unidadPlural={item.unidad_plural ?? undefined}
          />
        )}
      </div>
...
function ProductTimeline({
  ...
  const chartData = useMemo(() => {
    const semanas: Record<
      string,
      { semana: string; consumo: number; entradas: number }
    > = {};
    todosEventos.forEach((ev) => {
      ...
      const label = lunes.toLocaleDateString(APP_LOCALE, {
        day: "2-digit",
        month: "short",
      });
      if (!semanas[key])
        semanas[key] = { semana: label, consumo: 0, entradas: 0 };
      if (ev.tipo === "salida" || ev.tipo === "descarte")
        semanas[key].consumo += Math.round(ev.cantidad);
      if (ev.tipo === "entrada")
        semanas[key].entradas += Math.round(ev.cantidad);
    });
    ...
    <div className="space-y-4">
      ...
      {chartData.length > 1 && (
        <div className="bg-base-200/40 rounded-2xl p-3">
          <p className="text-[10px] font-bold uppercase tracking-wider opacity-30 mb-2">
            Consumo semanal (90 días)
          </p>
          <ResponsiveContainer width="100%" height={60}>
            ...
            >
              ...
              <RechartsTooltip
                ...
                formatter={(val) => [
                  formatCantidad(Number(val), unidad, unidadPlural),
                  "Consumo",
                ]}
              />
      ...
      <div className="flex gap-1.5 flex-wrap">
        {(
          [
            { key: "todos", label: "Todos", count: todosEventos.length },
            {
              key: "entradas",
              label: "Entradas",
              count: todosEventos.filter((e) => e.tipo === "entrada").length,
            },
            {
              key: "consumos",
              label: "Consumos",
              count: todosEventos.filter((e) => e.tipo === "salida").length,
            },
            {
              key: "descartes",
              label: "Descartes",
              count: todosEventos.filter((e) => e.tipo === "descarte").length,
            },
          ] as const
        ).map((f) => (
          <button
        ...
        <span className="ml-auto text-[10px] opacity-30 self-center">
          Últimos 90 días
        </span>
      </div>
                ...
                <div className="flex-1 min-w-0 bg-base-100/50 border border-base-200/50 rounded-2xl p-3 hover:bg-base-200/20 transition-colors">
                  ...
                  <div className="flex items-center gap-1.5 mb-1.5">
                    ...
                    >
                      {isEntrada ? "+" : "-"}
                      <CantidadConUnidad
                        qty={Math.round(ev.cantidad)}
                        unidad={ev.unidad_base_nombre ?? ""}
                        pluralUnidad={ev.unidad_base_nombre_plural ?? undefined}
                      />
                    </div>
                  </div>
                  {ev.notas && (
                    <div className="flex items-start gap-1.5 mb-2 p-2 bg-base-200/50 rounded-lg">
                      ...
                      <p className="text-[11px] leading-snug italic opacity-70">
                        {ev.notas}
                      </p>
                    </div>
                  )}
