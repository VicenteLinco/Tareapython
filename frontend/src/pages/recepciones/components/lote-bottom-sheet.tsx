...
export function LoteBottomSheet({
  open,
  productoNombre,
  prefillNumeroLote,
  onConfirm,
  onCancel,
}: LoteBottomSheetProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      const t = setTimeout(
        () => (loteLocked ? fechaRef.current : loteRef.current)?.focus(),
        150,
      );
  ...
  const canConfirm =
    numeroLote.trim().length > 0 && fechaVencimiento.length > 0;
