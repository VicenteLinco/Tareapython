...
export const useAuthStore = create<AuthState>()(
  persist(
    (set) => ({
      accessToken: typeof window !== "undefined" ? sessionStorage.getItem("lab-auth-access-token") : null,
      refreshToken: null,
      usuario: null,
      setTokens: (access, refresh) => {
        if (typeof window !== "undefined") {
          sessionStorage.setItem("lab-auth-access-token", access);
        }
      ...
      logout: () => {
        if (typeof window !== "undefined") {
          sessionStorage.removeItem("lab-auth-access-token");
        }
