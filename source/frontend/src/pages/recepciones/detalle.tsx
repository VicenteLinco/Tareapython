...
export default function RecepcionDetallePage() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () =>
      api
        .get<RecepcionDetalleResponse>(`/recepciones/${id}`)
        .then((r) => r.data),
    enabled: !!id,
  });
  ...
  function imprimirPDF() {
    ...
    const esConfirmada =
      recepcion.estado === "completa" || recepcion.estado === "confirmada";
...
<body>
  ...
  if (isLoading) {
    return <PageLoading label="Cargando recepción..." />;
  }
  ...
  const { recepcion, nota, foto_documento, foto_actualizada_at, detalle } =
    data;
  ...
  const fotoEsPosterior = (() => {
    ...
    return (
      new Date(foto_actualizada_at).getTime() -
        new Date(recepcion.created_at).getTime() >
      10 * 60 * 1000
    );
  })();
    ...
    <div className="space-y-5 max-w-5xl">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2 pl-10">
          ...
          {canOperate && !foto_documento && (
            <button
              className={cn(
                "btn btn-sm btn-primary shadow-md gap-2 font-bold px-5 hover:scale-[1.02] transition-all animate-pulse",
                uploadFotoMut.isPending && "loading",
              )}
        ...
        {confirmReplace && (
          <div className="ml-10 flex items-center gap-3 rounded-lg border border-warning bg-warning/10 px-4 py-2.5 text-sm">
            ...
            <button
              className={cn(
                "btn btn-sm btn-warning gap-1.5",
                uploadFotoMut.isPending && "loading",
              )}
      ...
      <div className="rounded-xl border border-base-200 bg-base-100 divide-y divide-base-200">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-0 divide-x divide-base-200">
          <div className="p-4">
            <p className="text-[10px] font-semibold uppercase tracking-wider opacity-40 mb-1">
              Proveedor
            </p>
          ...
          <span>
            ...
            {foto_actualizada_at && (
              <span className="opacity-60 ml-1">
                · {formatDate(foto_actualizada_at)}
              </span>
            )}
          </span>
        </div>
              ...
              <tbody>
                {detalle.map((item) => {
                  ...
                  return (
                    <tr
                      key={item.id}
                      className="hover:bg-base-200/30 border-base-200/60"
                    >
        ...
        >
          ...
          <LabelsSection
            lotesConfirmados={detalle.map((item) => ({
              ...
              cantidad_etiquetas: Math.max(
                1,
                Math.round(toNum(item.cantidad_presentaciones)),
              ),
            }))}
          />
