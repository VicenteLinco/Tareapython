...
use crate::dto::solicitud::{
    CreateSolicitudItem, EnvioProveedorView, ProveedorResumen, RegistrarEnvioInput,
    SolicitudDetalle, SolicitudDetalleItem, SolicitudResumen,
};
...
pub struct HorizonteFactores {
    pub ciclo_historico_dias: Option<i32>,
    pub n_pedidos_historico: i32,
    pub coeficiente_variacion: f64,
    pub multiplicador_variabilidad: f64,
    pub lead_time: i32,
}

#[derive(Debug, Serialize)]
pub struct HorizonteResponse {
    pub horizonte_sugerido: i32,
    pub razon: String,
    pub consumo_diario: f64,
    pub consumo_diario_forecast: f64,
    pub consumo_diario_planificacion: f64,
    pub tipo_estimacion_demanda: String,
    pub stock_actual: f64,
    pub stock_minimo: f64,
    pub precio_ultimo: Option<f64>,
    pub factores: HorizonteFactores,
}
...
/// Filtros de listado de solicitudes ya validados/clampados por el handler.
pub struct ListarSolicitudesParams {
    pub page: i64,
    pub per_page: i64,
    pub q: Option<String>,
...
impl SolicitudService {
    /// Detalle completo de una solicitud: cabecera, items, envios por proveedor y resumen.
    pub async fn obtener_detalle(pool: &PgPool, id: Uuid) -> Result<SolicitudDetalle, AppError> {
        ...
        let items = sqlx::query_as::<_, SolicitudDetalleItem>(
            r#"SELECT
                d.producto_id,
                COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)) as proveedor_id,
               ...
               LEFT JOIN proveedores prov ON prov.id = COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id))
        ...
        let envios = sqlx::query_as::<_, EnvioProveedorView>(
            r#"WITH proveedores_solicitud AS (
                   ...
                   LEFT JOIN presentaciones fallback_pres ON fallback_pres.producto_id = d.producto_id AND fallback_pres.activa = true
                   LEFT JOIN proveedores prov ON prov.id = COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id))
                   WHERE d.solicitud_id = $1 AND COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)) IS NOT NULL
                   GROUP BY COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)), prov.nombre
               )
               SELECT
                   ps.proveedor_id,
                   ps.proveedor_nombre,
    ...
    /// Listado paginado de solicitudes con filtros opcionales (texto, estado, proveedor).
    /// Devuelve la página de resultados y el total de filas que matchean.
    pub async fn listar(
        pool: &PgPool,
        params: ListarSolicitudesParams,
    ) -> Result<(Vec<SolicitudResumen>, i64), AppError> {
        ...
        if let Some(proveedor_id) = params.proveedor_id {
            count_builder.push(
                " AND EXISTS (SELECT 1 FROM solicitud_compra_detalle scd \
                 ...
                 WHERE scd.solicitud_id = s.id AND COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)) = ",
            );
        ...
        let mut list_builder: sqlx::QueryBuilder<sqlx::Postgres> = sqlx::QueryBuilder::new(
            r#"SELECT s.id, s.numero_documento, s.fecha_creacion, s.estado,
                      ...
                      COALESCE((
                          SELECT COUNT(DISTINCT COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)))::integer
                          FROM solicitud_compra_detalle scd
                          LEFT JOIN presentaciones pres ON pres.id = scd.presentacion_id
                          LEFT JOIN presentaciones fallback_pres ON fallback_pres.producto_id = scd.producto_id AND fallback_pres.activa = true
                          WHERE scd.solicitud_id = s.id AND COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)) IS NOT NULL
                      ...
                      (
                          ...
                          LEFT JOIN presentaciones pres ON pres.id = scd.presentacion_id
                          LEFT JOIN presentaciones fallback_pres ON fallback_pres.producto_id = scd.producto_id AND fallback_pres.activa = true
                          JOIN proveedores prov ON prov.id = COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id))
        ...
        list_builder.push_bind(params.per_page);
        ...
        Ok((solicitudes, total))
    }

    /// Borrador en curso del usuario, si existe (detalle completo).
    pub async fn obtener_borrador(
        pool: &PgPool,
        usuario_id: Uuid,
    ) -> Result<Option<SolicitudDetalle>, AppError> {
        ...
        match borrador_id {
            None => Ok(None),
            Some(id) => Ok(Some(Self::obtener_detalle(pool, id).await?)),
        }
    }
    ...
    /// Crea un borrador para el usuario. Si ya tiene uno en curso, reemplaza su
    /// contenido (un borrador por usuario). Devuelve el id del borrador.
    pub async fn crear_o_actualizar_borrador(
        pool: &PgPool,
        usuario_id: Uuid,
        nota: &Option<String>,
        ...
        if let Some(id) = borrador_existente {
            ...
            sqlx::query("UPDATE solicitudes_compra SET nota = $1 WHERE id = $2")
                ...
                .await?;
            for item in items {
                Self::insertar_item(&mut tx, id, item).await?;
            }
            tx.commit().await?;
            return Ok(id);
        }
        ...
        let solicitud_id = match insert_result {
            ...
            Err(sqlx::Error::Database(ref db_err)) if db_err.is_unique_violation() => {
                // Race condition: otra request insertó un borrador entre el SELECT y este INSERT.
                // Revertimos y devolvemos el borrador existente.
        ...
        for item in items {
            Self::insertar_item(&mut tx, solicitud_id, item).await?;
        }
    ...
    /// Reemplaza nota y detalle de un borrador. Solo aplica si está en estado borrador.
    pub async fn actualizar_borrador(
        pool: &PgPool,
        id: Uuid,
        nota: &Option<String>,
        items: &[CreateSolicitudItem],
    ...
    /// Confirma un borrador: valida que todos los items tengan proveedor, pasa a
    /// estado `guardada` e inicializa los envios por proveedor en `pendiente`.
    pub async fn guardar(pool: &PgPool, id: Uuid) -> Result<(), AppError> {
        let mut tx = pool.begin().await?;
        ...
        let sin_proveedor: i64 = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM solicitud_compra_detalle d
             LEFT JOIN presentaciones pres ON pres.id = d.presentacion_id
             LEFT JOIN presentaciones fallback_pres ON fallback_pres.producto_id = d.producto_id AND fallback_pres.activa = true
             WHERE d.solicitud_id = $1 AND COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)) IS NULL",
        )
        .bind(id)
        .fetch_one(&mut *tx)
        .await?;
        ...
        sqlx::query(
            "INSERT INTO solicitud_envios (solicitud_id, proveedor_id, estado)
             SELECT DISTINCT $1, COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)), 'pendiente'
             FROM solicitud_compra_detalle d
             LEFT JOIN presentaciones pres ON pres.id = d.presentacion_id
             LEFT JOIN presentaciones fallback_pres ON fallback_pres.producto_id = d.producto_id AND fallback_pres.activa = true
             WHERE d.solicitud_id = $1 AND COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)) IS NOT NULL
    ...
    /// Cierra una solicitud como completada. Requiere una recepción `completa` vinculada.
    pub async fn completar(pool: &PgPool, id: Uuid) -> Result<(), AppError> {
        let rows = sqlx::query(
    ...
    /// Cancela una solicitud con un motivo obligatorio.
    pub async fn cancelar(pool: &PgPool, id: Uuid, motivo: &str) -> Result<(), AppError> {
    ...
    /// Recalcula el estado de la solicitud según cuántos proveedores tienen su envío
    /// marcado como `enviado`. No toca estados terminales.
        ...
        let (total_provs, provs_enviados, fecha_max): (i64, i64, Option<DateTime<Utc>>) =
            sqlx::query_as(
                r#"WITH proveedores_items AS (
                       SELECT DISTINCT COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)) AS proveedor_id
                       FROM solicitud_compra_detalle d
                       LEFT JOIN presentaciones pres ON pres.id = d.presentacion_id
                       LEFT JOIN presentaciones fallback_pres ON fallback_pres.producto_id = d.producto_id AND fallback_pres.activa = true
                       WHERE d.solicitud_id = $1 AND COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)) IS NOT NULL
    ...
    /// Marca como enviada una solicitud guardada o parcialmente enviada: registra el
    /// método en la cabecera, inserta los envíos faltantes y recalcula el estado.
    pub async fn enviar(
        pool: &PgPool,
        id: Uuid,
        ...
        sqlx::query(
            "INSERT INTO solicitud_envios (solicitud_id, proveedor_id, estado, metodo_envio, fecha_envio, usuario_envio_id)
             SELECT DISTINCT $1, COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)), 'enviado', COALESCE($2, 'otro'), NOW(), $3
             FROM solicitud_compra_detalle d
             LEFT JOIN presentaciones pres ON pres.id = d.presentacion_id
             LEFT JOIN presentaciones fallback_pres ON fallback_pres.producto_id = d.producto_id AND fallback_pres.activa = true
             WHERE d.solicitud_id = $1 AND COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)) IS NOT NULL
        ...
        Self::recalcular_estado_solicitud(&mut tx, id).await?;
        tx.commit().await?;
        Ok(())
    }

    /// Registra el envío granular de un proveedor (optimistic locking por `version`) y
    /// recalcula el estado. Devuelve el detalle actualizado.
    pub async fn registrar_envio(
        pool: &PgPool,
        id: Uuid,
        input: &RegistrarEnvioInput,
        usuario_id: Uuid,
    ) -> Result<SolicitudDetalle, AppError> {
        if !matches!(
            input.metodo_envio.as_str(),
        ...
        let proveedor_presente: i64 = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*)
             ...
             LEFT JOIN presentaciones pres ON pres.id = d.presentacion_id
             LEFT JOIN presentaciones fallback_pres ON fallback_pres.producto_id = d.producto_id AND fallback_pres.activa = true
             WHERE d.solicitud_id = $1 AND COALESCE(get_proveedor_id_from_presentacion(pres.id), get_proveedor_id_from_presentacion(fallback_pres.id)) = $2",
        )
        .bind(id)
        ...
        .bind(id)
        .bind(input.proveedor_id)
        .bind(&input.metodo_envio)
        .bind(input.fecha_envio)
        .bind(usuario_id)
        .bind(&input.nota)
        .bind(input.version)
        .execute(&mut *tx)
        ...
        Self::obtener_detalle(pool, id).await
    }

    /// Revierte el envío de un proveedor a `pendiente` (optimistic locking por `version`)
    /// y recalcula el estado. Devuelve el detalle actualizado.
    pub async fn cancelar_envio(
        pool: &PgPool,
        id: Uuid,
        proveedor_id: i32,
        version: i32,
        ...
        if updated.rows_affected() == 0 {
            return Err(AppError::VersionConflict {
                esperada: version as i64,
                actual: version as i64 + 1,
            });
        }
    ...
    async fn load_forecast_config(pool: &PgPool) -> Result<ForecastConfig, AppError> {
    ...
    /// Recomendaciones de reposición: serie de consumo por producto + forecast en Rust.
    pub async fn recomendaciones(pool: &PgPool) -> Result<serde_json::Value, AppError> {
        // 1. Cargar configuración
        let cfg = Self::load_forecast_config(pool).await?;
        ...
        let rows = sqlx::query_as::<_, RecomendacionRow>(
            ...
            pedidos_en_vuelo AS (
                ...
                LEFT JOIN proveedores prov2 ON prov2.id = COALESCE(get_proveedor_id_from_presentacion(pres2.id), get_proveedor_id_from_presentacion(fallback_pres2.id))
            ...
            pres AS (
                SELECT DISTINCT ON (p.producto_id)
                    p.producto_id, p.id, p.nombre, p.nombre_plural, p.factor_conversion, op.proveedor_id, op.precio_adquisicion, p.sku
                FROM presentaciones p
                LEFT JOIN ofertas_proveedor op ON op.presentacion_id = p.id
                WHERE p.activa = true
            ...
            LEFT JOIN proveedores prov ON prov.id = get_proveedor_id_from_presentacion(pres.id)
        ...
        Ok(serde_json::json!({ "data": items }))
    }

    /// Horizonte de planificación sugerido para un producto/proveedor (forecast + cobertura).
    pub async fn horizonte(
        pool: &PgPool,
        producto_id: Uuid,
        proveedor_id: i32,
    ) -> Result<HorizonteResponse, AppError> {
        let cfg = Self::load_forecast_config(pool).await?;
