...
export function useConteoList() {
  ...
  const areaStockStatusQuery = useQuery({
    ...
    queryFn: async () => {
      const results = await Promise.all(
        areaIds.map((areaId) =>
          api
            .get<{ productos: unknown[] }>(`/stock/area/${areaId}`, {
              params: { per_page: 1 },
            })
            .then(
              (r) =>
                [
                  areaId,
                  r.data.productos.length > 0 ? "con-stock" : "sin-stock",
                ] as const,
            )
  ...
  const pendientesQuery = useQuery({
    ...
    queryFn: () =>
      api.get<AreaPendiente[]>("/conteo/pendientes").then((r) => r.data),
    staleTime: 60000,
  });
  ...
  const crearMutation = useMutation({
    mutationFn: (area_id: number) =>
      api
        .post<{ id: string; total_items: number }>("/conteo", { area_id })
        .then((r) => r.data),
    onSuccess: (data) => {
      ...
      if (data.total_items === 0) {
        notify.warning(
          "Esta área no tiene insumos en stock. El conteo se creó vacío.",
        );
      }
  ...
  const { data: configuracion } = useQuery({
    ...
    queryFn: () =>
      api
        .get<{ conteo_periodo_dias: number }>("/configuracion")
        .then((r) => r.data),
    staleTime: 300_000,
  });
  ...
  const handleCrearMultiple = async (areaIds: number[]) => {
    ...
    try {
      ...
      if (vacias > 0)
        notify.warning(
          `${vacias} área${vacias > 1 ? "s" : ""} sin stock en sistema`,
        );
    } catch (err) {
  ...
  const areaStockStatus: Record<number, AreaStockStatus> = {
    ...Object.fromEntries(
      areaIds.map((areaId) => [areaId, "loading" as AreaStockStatus]),
    ),
