...
function getZone(dias: number | null): {
  pct: number;
  color: string;
  label: string;
} {
  if (dias === null || dias < 0)
    return { pct: 0, color: "bg-base-300", label: "Sin datos" };
  /* zonas: 0-7 rojo, 8-30 amarillo, >30 verde — cap a 100% en 90 días */
  const pct = Math.min(100, (dias / 90) * 100);
  if (dias <= 7)
    return { pct, color: "bg-error", label: `${Math.round(dias)}d` };
  if (dias <= 30)
    return { pct, color: "bg-warning", label: `${Math.round(dias)}d` };
  return { pct, color: "bg-success", label: `${Math.round(dias)}d` };
}

export function AutonomiaBar({
  dias,
  className,
  showLabel = true,
}: AutonomiaBarProps) {
  ...
  return (
    <div className={cn("flex items-center gap-2", className)}>
      ...
      {showLabel && (
        <span className="t-caption tabular-nums w-8 text-right shrink-0">
          {label}
        </span>
      )}
    </div>
