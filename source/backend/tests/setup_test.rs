...
async fn setup_import_persists_price_without_explicit_provider(pool: PgPool) {
    ...
    assert_eq!(
        offers_count, 1,
        "price must be persisted in ofertas_proveedor"
    );

    let warning_found = result.advertencias.iter().any(|w| {
        w.codigo.as_deref() == Some("PRICE_DEFAULT_PROVIDER")
            || w.codigo.as_deref() == Some("COMMERCIAL_DATA_REQUIRES_PROVIDER")
    });
    assert!(
        warning_found,
        "diagnostic warning must be returned: {:?}",
        result.advertencias
    );
}
...
async fn setup_import_enforces_limits_rows_cols_cell_size(pool: PgPool) {
    ...
    let err_cell = inventario_lab_backend::services::setup_service::importar_catalogo(
        ...
        inventario_lab_backend::services::setup_service::ImportConfig {
            mapping: [
                ("nombre".into(), "nombre".into()),
                ("descripcion".into(), "descripcion".into()),
            ]
            .into_iter()
            .collect(),
            required_fields: vec![],
...
async fn setup_import_rejects_unknown_boolean_and_control_lote_tokens(pool: PgPool) {
    ...
    let codes = result
        ...
        .collect::<Vec<_>>();
    assert!(
        codes.contains(&"INVALID_BOOLEAN"),
        "must contain INVALID_BOOLEAN: {result:?}"
    );
    assert!(
        codes.contains(&"INVALID_CONTROL_LOTE"),
        "must contain INVALID_CONTROL_LOTE: {result:?}"
    );
}
...
#[sqlx::test(migrations = "./migrations")]
async fn setup_import_parses_synthetic_catalog_fixture(pool: PgPool) {
    let bytes = include_bytes!("fixtures/setup_catalog_synthetic.csv");
    let result = inventario_lab_backend::services::setup_service::importar_catalogo(
        &pool,
        bytes,
        inventario_lab_backend::services::setup_service::ImportConfig {
    ...
    assert!(result.valido, "{result:?}");
    assert_eq!(result.importados, 2, "{result:?}");
}
