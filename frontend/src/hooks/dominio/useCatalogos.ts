...
export function useActualizarUnidad() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: UpdateUnidadBasica }) =>
...
export function useCrearPresentacion() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ productoId, payload }: { productoId: string; payload: CreatePresentacion }) =>
    ...
    onSuccess: (_data, { productoId }) => {
      ...
      qc.invalidateQueries({ queryKey: catalogosKeys.productos.detail(productoId) })
