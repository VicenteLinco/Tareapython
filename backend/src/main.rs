...
async fn main() {
    ...
    // Iniciar el Job de Recalculo de Promedio de Uso Mensual en background
    let pool_for_promedio_job = pool.clone();
    tokio::spawn(async move {
        crate::services::promedio_job::start_promedio_recalculo_job(pool_for_promedio_job).await;
    });
