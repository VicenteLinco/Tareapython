...
export function classifyConsumoScan({ numeroLote, loteEnArea, existeEnStock }: ConsumoScanInput): ConsumoScanOutcome {
...
export function findLoteByNumero<T extends { numero_lote: string }>(lotes: T[], numeroLote: string): T | null {
  const target = numeroLote.trim().toUpperCase()
  return lotes.find(l => l.numero_lote.trim().toUpperCase() === target) ?? null
}
