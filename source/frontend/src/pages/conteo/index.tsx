...
const ESTADO_CONFIG = {
  borrador: {
    ...
    tooltip: "Sesión creada pero no iniciada. Puedes editarla o eliminarla.",
  },
  en_progreso: {
    label: "En progreso",
    icon: Clock,
    className: "badge-info",
    tooltip: "Sesión activa. Se están registrando conteos.",
  },
  confirmado: {
    label: "Confirmado",
    icon: CheckCircle2,
    className: "badge-success",
    tooltip: "Sesión cerrada. Los ajustes ya se aplicaron al stock.",
  },
  cancelado: {
    label: "Cancelado",
    icon: XCircle,
    className: "badge-ghost",
    tooltip: "Sesión cancelada. No se aplicaron ajustes.",
  },
} as const;
...
function getAreaUrgencia(
  area: { conteo_frecuencia_dias: number },
  pendiente: { dias_desde_ultimo: number | null } | undefined,
  periodoMax: number,
): AreaUrgencia {
  // Si el área no aparece en la lista de pendientes, fue contada recientemente y está al día
  if (pendiente === undefined)
    return { pct: 0, color: "success", label: "Al día" };
  const periodo =
    area.conteo_frecuencia_dias > 0 ? area.conteo_frecuencia_dias : periodoMax;
  const dias = pendiente.dias_desde_ultimo;
  if (dias === null)
    return { pct: 100, color: "error", label: "Nunca contada" };
  const pct = Math.min((dias / periodo) * 100, 120);
  if (pct >= 100)
    return {
      pct,
      color: "error",
      label: `${Math.round(dias)}d · límite ${periodo}d`,
    };
  if (pct >= 70)
    return {
      pct,
      color: "warning",
      label: `${Math.round(dias)} de ${periodo} días`,
    };
  return {
    pct,
    color: "success",
    ...
    <div className="p-4 max-w-2xl mx-auto pb-24">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <KeyboardLegend
            shortcuts={[
              ...
              {
                keys: ["Esc"],
                description: "Cerrar modal / limpiar filtro de área",
              },
            ]}
          />
          <button
      ...
      {pendientes.length > 0 ? (
        <div className="mb-5 bg-base-100 border border-base-200 rounded-xl overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-base-200 bg-base-200/40">
            ...
            <span className="text-sm font-bold">
              {pendientes.length} área{pendientes.length !== 1 ? "s" : ""}{" "}
              pendiente{pendientes.length !== 1 ? "s" : ""} de contar
            </span>
          </div>
          <div className="divide-y divide-base-200">
            {pendientes
              ...
              .sort(
                (a, b) =>
                  (b.dias_desde_ultimo ?? 9999) - (a.dias_desde_ultimo ?? 9999),
              )
              .map((p) => {
                ...
                const isAtrasada =
                  dias === null || dias > (p.frecuencia_dias ?? 30);
                const isProxima =
                  !isAtrasada &&
                  dias !== null &&
                  dias >= (p.frecuencia_dias ?? 30) * 0.7;
                const urgenciaLabel =
                  dias === null
                    ? "Nunca contada"
                    : isAtrasada
                ...
                return (
                  <div
                    key={p.area_id}
                    className="flex items-center justify-between px-4 py-2.5 hover:bg-base-200/30 transition-colors"
                  >
        ...
        <div className="mb-5 bg-success/5 border border-success/20 rounded-xl px-4 py-3 flex items-center gap-2">
          ...
          <span className="text-sm text-success font-medium">
            Todas las áreas al día en conteo
          </span>
        </div>
      )}
      ...
      <div className="flex flex-col gap-2 mb-4">
        <select
          ...
          onChange={(e) =>
            actions.setArea(e.target.value ? Number(e.target.value) : null)
          }
        >
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {(
            ["", "borrador", "en_progreso", "confirmado", "cancelado"] as const
          ).map((e) => (
            <button
              key={e}
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          ...
          {sesiones?.data.length === 0 && <EmptyState contexto="sin_conteos" />}
        </div>
      )}
            ...
            <div className="space-y-2 max-h-72 overflow-y-auto mb-4">
              {areas
                ...
                .map((a) => {
                  ...
                  const isCheckingStock = stockStatus === "loading";
                  const sinStock = stockStatus === "sin-stock";
                  const urgencia = getAreaUrgencia(
                    a,
                    pendiente,
                    periodoGlobalDias,
                  );
                  ...
                  return (
                    <label
                      ...
                      className={cn(
                        ...
                        sinStock || isCheckingStock
                          ? "opacity-40 cursor-not-allowed bg-base-200/50"
                          : selected
                    ...
                    >
                      <input
                        ...
                        onChange={() =>
                          !sinStock && !isCheckingStock && toggleArea(a.id)
                        }
                      />
                          ...
                          <>
                            <div className="w-full bg-base-200 rounded-full h-1.5 mb-1">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(urgencia.pct, 100)}%`,
                                }}
                              />
                            </div>
                            <span
      ...
      {sesion.estado !== "cancelado" && (
        <div className="mt-3">
          <div className="flex justify-between text-xs opacity-60 mb-1">
            <span>
              {sesion.items_contados} / {sesion.total_items} ítems
            </span>
            <span>{progreso}%</span>
          </div>
