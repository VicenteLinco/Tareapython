...
import type { PaginatedSesiones, SesionConteo, ConteoDetalle } from "@/types";
...
export async function detalleConteoCompleto(
  sesionId: string,
...
export async function guardarItemsConteo(
  sesionId: string,
...
export async function confirmarConteo(
  sesionId: string,
  payload?: ConfirmarConteoRequest,
): Promise<ConfirmarConteoResponse> {
  const { data } = await api.post<ConfirmarConteoResponse>(
    `/conteo/${sesionId}/confirmar`,
    payload ?? {},
  );
  return data;
}
