...
import type { UpdateSolicitudRequest, RegistrarEnvioInput, CancelarEnvioInput } from '@/types'
...
export function useActualizarSolicitud() {
  ...
  return useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: UpdateSolicitudRequest }) =>
...
export function useCancelarSolicitud() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, payload }: { id: string; payload?: CancelarSolicitudRequest }) =>
...
export function useRegistrarEnvio() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: RegistrarEnvioInput }) =>
