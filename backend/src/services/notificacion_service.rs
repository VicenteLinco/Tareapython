...
pub async fn listar(
    ...
    let total: i64 = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM notificaciones WHERE usuario_id = $1")
        .bind(usuario_id)
        .fetch_one(pool)
        .await?;
...
pub async fn obtener_conteo_no_leidas(
    ...
    let count: i64 = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM notificaciones WHERE usuario_id = $1 AND leido = false")
        .bind(usuario_id)
        .fetch_one(pool)
        .await?;

    Ok(UnreadCountResponse { conteo: count as i32 })
}
...
pub async fn marcar_leida(
    ...
    let rows_affected = sqlx::query("UPDATE notificaciones SET leido = true WHERE id = $1 AND usuario_id = $2")
...
pub async fn crear_para_admins(
    ...
    let admins: Vec<Uuid> = sqlx::query_scalar::<_, Uuid>("SELECT id FROM usuarios WHERE rol = 'admin' AND deleted_at IS NULL")
        .fetch_all(pool)
        .await?;
