...
#[sqlx::test(migrations = "./migrations")]
async fn test_alertas_multiples_condiciones(pool: PgPool) {
    let token = common::admin_access_token(&pool).await;
    let app = common::test_app(pool.clone());
    ...
    sqlx::query(
        "INSERT INTO par_level_config (producto_id, area_id, stock_minimo, stock_maximo, safety_stock, metodo) \
         VALUES ($1, 1, 100, 500, 0.0, 'manual') \
         ON CONFLICT (producto_id, area_id) DO UPDATE SET stock_minimo = EXCLUDED.stock_minimo, stock_maximo = EXCLUDED.stock_maximo"
    )
    .bind(producto_id)
    .execute(&pool)
    .await
    ...
    // Crear dos lotes mediante recepción:
    // 1. Lote expirado: 10 unidades (1 caja)
    // 2. Lote por vencer (en 10 días): 5 unidades (0.5 caja)
    let fecha_vto_por_vencer = (chrono::Utc::now() + chrono::Duration::days(10))
        .format("%Y-%m-%d")
        .to_string();

    let (_, _json_rec) = common::post_json_idempotent(
        &app,
        "/api/v1/recepciones",
        &token,
        serde_json::json!({
            ...
            "detalle": [
                {
                    "producto_id": producto_id,
                    "numero_lote": "LOTE-EXPIRADO",
                    "fecha_vencimiento": "2026-01-01",
                    "presentacion_id": presentacion_id,
                    "cantidad_presentaciones": 1.0,
                    "area_destino_id": 1,
                },
                {
                    "producto_id": producto_id,
                    "numero_lote": "LOTE-POR-VENCER",
                    "fecha_vencimiento": fecha_vto_por_vencer,
                    "presentacion_id": presentacion_id,
                    "cantidad_presentaciones": 0.5,
                    "area_destino_id": 1,
                }
            ]
        }),
    ...
    // Obtener lote_id de los dos lotes
    let lote_expirado_id: Uuid = sqlx::query_scalar(
        "SELECT id FROM lotes WHERE producto_id = $1 AND numero_lote = 'LOTE-EXPIRADO'",
    )
    .bind(producto_id)
    .fetch_one(&pool)
    .await
    ...
    // Consultar alertas generales
    let (status, json_alertas) = common::get_json(&app, "/api/v1/stock/alertas", &token).await;
    assert_eq!(status, StatusCode::OK);

    let resumen = &json_alertas["resumen"];
    // 1. Debe estar en bajo_minimo (usable = 5 <= min_manual 100)
    assert!(resumen["bajo_minimo"].as_i64().unwrap() >= 1);
    // 2. Debe estar en vencido (porque tiene vencido)
    assert!(resumen["vencido"].as_i64().unwrap() >= 1);

    // Ahora hacemos el descarte del lote vencido
    let (status_desc, _) = common::post_json_idempotent(
        &app,
        "/api/v1/descartes",
        &token,
        serde_json::json!({
            "items": [{
                "lote_id": lote_expirado_id,
                "area_id": 1,
                "cantidad": 10,
                "tipo": "DESCARTE_VENCIDO",
                "nota": "Descarte lote expirado de prueba"
            }]
        }),
        &Uuid::new_v4().to_string(),
    )
    .await;
    assert_eq!(status_desc, StatusCode::CREATED);

    // Consultar alertas generales de nuevo
    let (status2, json_alertas2) = common::get_json(&app, "/api/v1/stock/alertas", &token).await;
    assert_eq!(status2, StatusCode::OK);

    let resumen2 = &json_alertas2["resumen"];
    // 1. Sigue bajo_minimo (usable = 5)
    assert!(resumen2["bajo_minimo"].as_i64().unwrap() >= 1);
    // 2. Ya NO está en vencido (se descartó)
    assert_eq!(resumen2["vencido"].as_i64().unwrap(), 0);
    // 3. Ahora SÍ debe estar en vencimiento (riesgo_venc ya no está tapado por vencido)
    assert!(resumen2["vencimiento"].as_i64().unwrap() >= 1);
}
