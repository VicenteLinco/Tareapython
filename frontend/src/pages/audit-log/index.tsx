...
function groupByDay(
  logs: AuditLogItem[],
): { label: string; items: AuditLogItem[] }[] {
...
function DiffView({ log }: { log: AuditLogItem }) {
  ...
  const data =
    log.accion === "CREATE" ? log.datos_nuevos : log.datos_anteriores;
  if (!data) return null;
        ...
        <table className="w-full text-xs">
          <tbody className="divide-y divide-base-200">
            {entries.map(([campo, valor]) => (
              <tr key={campo} className="hover:bg-base-200/20">
                <td className="px-3 py-2 font-mono text-base-content/50 w-1/3">
                  {campo}
                </td>
      ...
      {isLoading ? (
        <div className="flex flex-col gap-3">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className="h-14 bg-base-200 animate-pulse rounded-2xl"
            />
          ))}
        </div>
      ) : logs.length === 0 ? (
        <EmptyState
          ...
          descripcion="No se encontraron registros para los filtros aplicados."
        />
      ) : (
        <div className="flex flex-col gap-6">
          {groups.map(({ label, items }) => (
            <div key={label} className="flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-1.5">
                {items.map((log) => {
                  const name =
                    extractName(log.datos_nuevos) ??
                    extractName(log.datos_anteriores);
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <div
                          className={`p-2 rounded-xl shrink-0 ${actionColors(log.accion)}`}
                        >
