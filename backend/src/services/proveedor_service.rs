...
pub async fn listar(pool: &PgPool, params: ProveedorQuery) -> Result<Vec<Proveedor>, AppError> {
    ...
    if let Some(q) = params.q {
        ...
        sqlx::query_as::<_, Proveedor>(
            "SELECT id, nombre, contacto, telefono, email, icono, activa, dias_despacho_aereo, dias_despacho_tierra, version \
...
pub async fn crear(
    ...
    let proveedor = sqlx::query_as::<_, Proveedor>(
        "INSERT INTO proveedores (nombre, contacto, telefono, email, icono, dias_despacho_aereo, dias_despacho_tierra) \
         ...
         RETURNING id, nombre, contacto, telefono, email, icono, activa, dias_despacho_aereo, dias_despacho_tierra, version",
    )
    .bind(&nombre)
    .bind(&req.contacto)
    .bind(&req.telefono)
...
pub async fn actualizar(
    ...
    let anterior = sqlx::query_as::<_, Proveedor>("SELECT id, nombre, contacto, telefono, email, icono, version FROM proveedores WHERE id = $1")
        .bind(id)
        .fetch_optional(pool)
        .await?
