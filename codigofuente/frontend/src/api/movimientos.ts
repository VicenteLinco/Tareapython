...
export async function listarMovimientos(
  ...
  const { data } = await api.get<PaginatedResponse<Movimiento>>(
    "/movimientos",
    { params },
  );
  return data;
...
export async function tendenciasMovimientos(
  params?: TendenciasQuery,
): Promise<TendenciasResponse> {
  const { data } = await api.get<TendenciasResponse>(
