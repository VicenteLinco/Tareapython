...
export function ProveedorBanner({
  proveedores,
  disponibles,
  quiebresCount,
  onQuitar,
  ...
  const candidatos = disponibles.filter(
    (p) => !proveedores.some((sel) => sel.id === p.id),
  );
    ...
    <div className="flex items-center gap-3 px-5 py-3 bg-primary/5 border border-primary/15 rounded-2xl shrink-0">
      <div className="flex-1 min-w-0 space-y-2">
        ...
        <p className="text-[10px] opacity-45 font-bold uppercase tracking-wide">
          {quiebresCount} recomendacion{quiebresCount !== 1 ? "es" : ""}{" "}
          visibles. El carrito se mantiene al cambiar filtros.
        </p>
      </div>
