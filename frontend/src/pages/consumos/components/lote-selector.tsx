...
export function LoteSelector({ lotes, cargandoLotes, loteElegidoId, unidad, unidad_plural, onChange }: LoteSelectorProps) {
  ...
  if (cargandoLotes) {
    return (
      <div className="flex items-center gap-1.5 h-7">
        ...
        <span className="text-[11px] text-base-content/40">Cargando lotes…</span>
      </div>
    )
  }
  ...
  if (lotes.length === 0) {
    return (
      <span className="text-[11px] text-warning/80 font-medium">Sin lotes disponibles</span>
    )
  }
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-base-content/40 font-normal text-[11px]">
                    {formatCantidad(l.stock, unidad, unidad_plural)} · vence {formatDate(l.fecha_vencimiento)}
                  </div>
                </div>
