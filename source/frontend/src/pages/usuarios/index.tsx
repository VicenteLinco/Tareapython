...
const ROL_CONFIG = {
  ...
  consulta: {
    label: "Consulta",
    badgeCls: "bg-base-200 text-base-content/60 border border-base-300",
    icon: BookOpen,
    avatarBg: "bg-base-200",
    avatarFg: "text-base-content/50",
  },
} as const;

function RolBadge({ rol }: { rol: string }) {
  const c = ROL_CONFIG[rol as keyof typeof ROL_CONFIG] ?? {
    label: rol,
    badgeCls: "bg-base-200 text-base-content/60 border border-base-300",
  };
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold tracking-wide ${c.badgeCls}`}
    >
      {c.label}
    </span>
  );
...
function RolAvatar({ rol }: { rol: string }) {
  ...
  return (
    <div
      className={`w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0 ${c.avatarBg}`}
    >
...
function ModalUsuario({ open, onClose, usuario }: ModalUsuarioProps) {
  ...
  const updateMut = useMutation({
    ...
    onError: (err: unknown) => {
      ...
      if (apiErr.response?.status === 409) {
        notify.error(
          "Conflicto: El usuario fue modificado por otro administrador. Recarga la lista.",
        );
      } else {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (isEdit) {
      ...
      if (!form.password) {
        notify.error("La contraseña es obligatoria");
        return;
      }
      createMut.mutate({
        nombre: form.nombre,
    ...
    <div className="modal modal-open">
      <div className="modal-box max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="form-control">
            <label className="label">
              <span className="label-text">WhatsApp (Opcional)</span>
            </label>
            <input
              type="text"
              className="input input-bordered input-sm"
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, whatsapp_phone: e.target.value }))
              }
            />
          </div>
...
function ModalPassword({
  ...
  const mut = useMutation({
    mutationFn: () =>
      api.post(`/usuarios/${userId}/reset-password`, {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (password.length < 8) {
      notify.error("Mínimo 8 caracteres");
      return;
    }
    mut.mutate();
  }
...
export default function UsuariosPage() {
  ...
  const [editando, setEditando] = useState<UsuarioResponse | null | undefined>(
    undefined,
  ); // undefined = cerrado
  const [passwordId, setPasswordId] = useState<string | null>(null);
  const [toggleTarget, setToggleTarget] = useState<UsuarioResponse | null>(
    null,
  );
  ...
  return (
    <div>
              ...
              <div className="p-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0 pt-0.5">
                    ...
                    {u.whatsapp_phone && (
                      <p
                        className="text-[11px] text-success flex items-center gap-1 mt-1 font-medium truncate"
                        title={u.whatsapp_phone}
                      >
      ...
      {/* Modal cambiar contraseña */}
      <ModalPassword userId={passwordId} onClose={() => setPasswordId(null)} />
