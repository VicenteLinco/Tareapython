...
export async function imprimirEtiquetas(
  ...
  opciones: OpcionesImprenta = { formato: 'rollo', rolloTamano: '50x25', mostrarBordes: true }
): Promise<void> {
  ...
  if (isRollo) {
    ...
    if (size === '50x25') {
      paperWidth = 50; paperHeight = 25
    } else if (size === '40x30') {
      paperWidth = 40; paperHeight = 30
    } else if (size === '60x40') {
      paperWidth = 60; paperHeight = 40
    } else if (size === '80x50') {
      paperWidth = 80; paperHeight = 50
    } else {
    ...
    if (size === 'carta') {
      paperWidth = 215.9; paperHeight = 279.4
    } else if (size === 'oficio') {
      paperWidth = 216; paperHeight = 330
    } else if (size === 'a4') {
    ...
    if (diseno === '3x10') {
      sheetCols = 3; sheetRows = 10
    } else if (diseno === '3x8') {
      sheetCols = 3; sheetRows = 8
    } else if (diseno === '4x10') {
      sheetCols = 4; sheetRows = 10
    } else {
  ...
  for (const lote of lotes) {
    ...
    const nombreCorto = lote.producto_nombre.length > maxChars
  ...
  const infoFont = Math.min(11, Math.max(5.0, cellHeight * 0.20))
