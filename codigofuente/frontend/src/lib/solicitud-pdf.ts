...
export async function exportarSolicitudPDF(
  options: SolicitudPdfOptions,
): Promise<void> {
  const {
    numero_documento,
    fecha_creacion,
  ...
  const nombreTrunc = doc.splitTextToSize(
    usuario_nombre.toUpperCase(),
    72,
  )[0] as string;
  ...
  const renderTablaItems = (
    ...
    autoTable(doc, {
      ...
      head: [["#", "Producto", "Cantidad", "P. Unit. Base", "Total Neto"]],
      body: itemsToRender.map((item, index) => {
        ...
        const presLabel = usaPresentacion
          ? item.cantidad_presentaciones === 1
            ? item.presentacion_nombre!
            : (item.presentacion_nombre_plural ??
              item.presentacion_nombre + "s")
          : "";
        const baseQty = Math.round(item.cantidad_sugerida);
        const baseUnitLabel =
          baseQty === 1 ? item.unidad : (item.unidad_plural ?? item.unidad);
        ...
        const precioPres =
          usaPresentacion && item.factor_conversion
            ? precioBase * item.factor_conversion
            : null;
        const qty = usaPresentacion
          ? item.cantidad_presentaciones!
        ...
        const precioDisplay = hasPrice ? fmtMonto(precioBase) : "—";
  ...
  if (grupos && grupos.length > 1) {
    ...
    for (const grupo of grupos) {
      ...
      doc.text(subtotalText, doc.internal.pageSize.width - 14, currentY + 4, {
        align: "right",
      });
      currentY += 10;
    }
  ...
  const signY = firmasStartY + 42 > H - 20 ? (doc.addPage(), 45) : firmasStartY;
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
      "Documento generado electrónicamente por el Sistema de Gestión de Inventario.",
      15,
      H - 10,
    );
  ...
  doc.save(
    `SOLICITUD_${numero_documento}_${formatDate(fecha_creacion).replace(/\//g, "-")}.pdf`,
  );
}
