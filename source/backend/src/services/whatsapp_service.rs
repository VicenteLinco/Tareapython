...
pub async fn registrar_recepcion_tool(
    ...
    let mut provider_id: Option<i32> = sqlx::query_scalar::<_, i32>(
        r#"SELECT op.proveedor_id
           FROM ofertas_proveedor op
           JOIN presentaciones pres ON pres.id = op.presentacion_id
           JOIN proveedores prov ON prov.id = op.proveedor_id
