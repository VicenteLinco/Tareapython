...
pub async fn send_whatsapp_reply(
    ...
    let settings = load_whatsapp_settings(pool)
        .await
        .unwrap_or_else(|_| WhatsappSettings {
...
pub async fn execute_tool(
    ...
    match tool_name {
        "buscar_stock" => {
            let args: BuscarStockArgs = serde_json::from_value(args).map_err(|e| {
                AppError::Internal(format!("Invalid arguments for buscar_stock: {}", e))
            })?;
            let res = execute_buscar_stock(pool, user, args).await?;
            Ok(serde_json::to_value(res).map_err(|e| AppError::Internal(e.to_string()))?)
        }
        "registrar_ingreso" => {
            let args: RegistrarIngresoArgs = serde_json::from_value(args).map_err(|e| {
                AppError::Internal(format!("Invalid arguments for registrar_ingreso: {}", e))
            })?;
            let res = execute_registrar_ingreso(pool, user, args).await?;
            Ok(serde_json::to_value(res).map_err(|e| AppError::Internal(e.to_string()))?)
        }
        "registrar_consumo" => {
            let args: RegistrarConsumoArgs = serde_json::from_value(args).map_err(|e| {
                AppError::Internal(format!("Invalid arguments for registrar_consumo: {}", e))
            })?;
            let res = execute_registrar_consumo(pool, user, args).await?;
            Ok(res)
        }
        "crear_solicitud_compra" => {
            let args: CrearSolicitudCompraArgs = serde_json::from_value(args).map_err(|e| {
                AppError::Internal(format!(
                    "Invalid arguments for crear_solicitud_compra: {}",
                    e
                ))
            })?;
...
pub async fn webhook_handler(
    ...
    let wa_settings = load_whatsapp_settings(&state.pool)
        .await
        .unwrap_or_else(|_| WhatsappSettings {
    ...
    let msg = if headers.contains_key("X-Twilio-Signature") {
        ...
        let post_params: Vec<(String, String)> =
        ...
        let twilio_payload: TwilioWebhook =
            serde_urlencoded::from_bytes(&body).map_err(|_| StatusCode::BAD_REQUEST)?;
        ...
        let openwa_payload: OpenwaWebhook =
    ...
    let exists = whatsapp_service::webhook_log_exists(&state.pool, &msg.id)
        .await
        .unwrap_or(false);
...
pub async fn process_message_async(state: AppState, msg: WebhookMessage) -> Result<(), AppError> {
    ...
    let user = match user_res {
        Ok(Some(u)) => u,
        Ok(None) => {
            let access_denied_msg =
                "Acceso denegado: Su número de WhatsApp no está registrado o está inactivo.";
            ...
            .await;
            let _ =
                send_whatsapp_reply(&state.pool, &state.config, &msg.from, access_denied_msg).await;
            return Ok(());
        }
        Err(e) => {
...
mod tests {
    ...
    fn test_normalize_phone() {
        ...
        assert_eq!(
            normalize_phone("whatsapp:+56912345678@c.us"),
            "+56912345678"
        );
    ...
    async fn test_execute_tool_validation_and_early_errors(pool: sqlx::PgPool) {
        ...
        let rbac_fail = execute_registrar_ingreso(&pool, &normal_user, registrar_args.clone())
            .await
            .unwrap();
        ...
        let qty_fail = execute_registrar_ingreso(&pool, &admin_user, invalid_qty_args)
            .await
            .unwrap();
        ...
        let scale_fail = execute_registrar_ingreso(&pool, &admin_user, invalid_scale_args)
            .await
            .unwrap();
        ...
        let past_fail = execute_registrar_ingreso(&pool, &admin_user, past_expiry_args)
            .await
            .unwrap();
        ...
        let format_fail = execute_registrar_ingreso(&pool, &admin_user, invalid_format_args)
            .await
            .unwrap();
        ...
        let area_fail = execute_registrar_ingreso(&pool, &admin_user, non_existent_area_args)
            .await
            .unwrap();
        ...
        let sol_fail = execute_crear_solicitud_compra(&pool, &admin_user, sol_args)
            .await
            .unwrap();
    ...
    async fn test_execute_tool_success_routes(pool: sqlx::PgPool) {
        ...
        sqlx::query(
            "INSERT INTO presentaciones (producto_id, nombre, nombre_plural, factor_conversion, activa) \
             VALUES ($1, 'Unidad', 'Unidades', 1.0, true)",
        )
        .bind(product_id)
        .execute(&pool)
        .await
        ...
        let result = execute_registrar_ingreso(&pool, &admin_user, registrar_args)
            .await
            .unwrap();
        ...
        let result_sol = execute_crear_solicitud_compra(&pool, &admin_user, sol_args)
            .await
            .unwrap();
        ...
        let search_res = execute_buscar_stock(&pool, &admin_user, search_args)
            .await
            .unwrap();
        ...
        assert_eq!(
            search_res.items[0].stock_total,
    ...
    async fn test_parallel_execution_and_failure_isolation(pool: sqlx::PgPool) {
        ...
        let res1 = execute_tool(&pool, &admin_user, "registrar_ingreso", args1)
            .await
            .unwrap();
        let res2 = execute_tool(&pool, &admin_user, "registrar_ingreso", args2)
            .await
            .unwrap();
        ...
        let cres1 = execute_tool(&pool, &admin_user, "registrar_consumo", consume_args1)
            .await
            .unwrap();
        let cres2 = execute_tool(&pool, &admin_user, "registrar_consumo", consume_args2)
            .await
            .unwrap();
    ...
    async fn test_rbac_global_search_for_tecnologo(pool: sqlx::PgPool) {
        ...
        let area_exists2 =
            sqlx::query_scalar::<_, bool>("SELECT EXISTS(SELECT 1 FROM areas WHERE id = 2)")
                .fetch_one(&pool)
                .await
                .unwrap();
        if !area_exists2 {
        ...
        let res = execute_buscar_stock(&pool, &tec_user, search_args)
            .await
            .unwrap();
    ...
    async fn test_interactive_consumption_flow(pool: sqlx::PgPool) {
        ...
        let res = execute_registrar_consumo(&pool, &admin_user, args)
            .await
            .unwrap();

        assert_eq!(
            res.get("status").unwrap().as_str().unwrap(),
            "needs_lote_selection"
        );
        assert_eq!(
            res.get("producto_nombre").unwrap().as_str().unwrap(),
            "Interactive Product"
        );
        ...
        let res_selected = execute_registrar_consumo(&pool, &admin_user, args_selected)
            .await
            .unwrap();
        assert_eq!(
            res_selected.get("status").unwrap().as_str().unwrap(),
            "success"
        );
        assert!(
            res_selected
        ...
        let stock_l14: rust_decimal::Decimal =
            sqlx::query_scalar("SELECT cantidad FROM stock WHERE lote_id = $1 AND area_id = 2")
                .bind(l2_id)
                .fetch_one(&pool)
                .await
                .unwrap();
        ...
        let stock_l12: rust_decimal::Decimal =
