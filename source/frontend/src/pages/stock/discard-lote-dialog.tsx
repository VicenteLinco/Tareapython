...
export function DiscardLoteDialog({
  open,
  loteId,
  numeroLote,
  productoNombre,
  ...
  useEffect(() => {
    if (stockPorArea.length > 0) {
      if (
        defaultAreaId &&
        stockPorArea.some((s) => s.area_id === defaultAreaId)
      ) {
  ...
  const maxCantidad = areaId
    ? (stockPorArea.find((s) => s.area_id === Number(areaId))?.cantidad ?? 0)
    : 0;
        ...
        >
          ...
          <div className="text-sm">
            ...
            <p className="opacity-50 text-xs">
              Lote: <span className="font-mono">{numeroLote}</span>
            </p>
            {isExpired && (
              <p className="text-error font-bold text-[10px] uppercase mt-1">
                Lote Vencido el {fechaVencimiento}
              </p>
            )}
          </div>
        </div>
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold opacity-30">
                    MAX: {Math.round(maxCantidad)}
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={
                  isExpired
                    ? "Opcional: Detalles del retiro..."
                    : "Obligatorio: Describa el motivo del descarte..."
                }
                value={nota}
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              >
                {mutation.isPending
                  ? "Procesando..."
                  : isExpired
                    ? "Confirmar Baja por Vencimiento"
                    : "Confirmar Descarte Manual"}
              </Button>
            </div>
          </form>
        )}
