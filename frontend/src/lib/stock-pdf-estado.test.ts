...
import { getEstado, esSinStock, esBajo, esPorVencer30, ESTADO_LABEL } from './stock-pdf-estado'
...
describe('getEstado — taxonomía de estados del PDF', () => {
  ...
  it('prioriza sin stock sobre vencido', () => {
    expect(getEstado(item({ estado_cantidad: 'agotado', estado_vencimiento: 'vencido' }))).toBe('sin_stock')
  })

  it('clasifica vencido cuando hay stock usable', () => {
    expect(getEstado(item({ estado_cantidad: 'normal', estado_vencimiento: 'vencido' }))).toBe('vencido')
  })

  it('clasifica por vencer dentro de 30 días', () => {
    const it1 = item({ estado_cantidad: 'normal', proximo_vencimiento: inDays(15) })
    expect(esPorVencer30(it1)).toBe(true)
    expect(getEstado(it1)).toBe('vencer')
  })

  it('NO marca por vencer un vencimiento lejano (> 30 días)', () => {
    const it1 = item({ estado_cantidad: 'normal', proximo_vencimiento: inDays(120) })
  ...
  it('un vencimiento ya pasado no cuenta como "por vencer 30d"', () => {
    expect(esPorVencer30(item({ proximo_vencimiento: inDays(-5) }))).toBe(false)
  })

  it('prioriza por vencer sobre bajo cuando coinciden', () => {
    const it1 = item({ estado_cantidad: 'reponer', proximo_vencimiento: inDays(10) })
    expect(getEstado(it1)).toBe('vencer')
  })
