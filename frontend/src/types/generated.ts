...
export type ControlLote = "trazable" | "con_vto" | "simple"
export type EstadoCatalogo = "pendiente_aprobacion" | "aprobado"
export type OrigenRegistro = "manual" | "api_regulatoria" | "guia_pdf"
export type Area = { id: number; nombre: string; es_bodega: boolean; activa: boolean; created_at: string; conteo_frecuencia_dias: number; version: number; total_items_stock: number | null; es_virtual: boolean }
export type Categoria = { id: number; nombre: string; descripcion: string | null; created_at: string; version: number }
export type UnidadBasica = { id: number; nombre: string; nombre_plural: string; version: number; categoria: string }
...
export type Producto = { id: string; codigo_interno: string; nombre: string; descripcion: string | null; categoria_id: number | null; unidad_base_id: number; ubicacion: string | null; temperatura_almacenamiento: string | null; requiere_cadena_frio: boolean; dias_estabilidad_abierto: number | null; clase_riesgo: string | null; fabricante: string | null; deleted_at: string | null; activo: boolean; control_lote: ControlLote; estado_catalogo: EstadoCatalogo; origen_registro: OrigenRegistro; version: number; created_at: string; updated_at: string; proveedor_id: number | null; sku: string | null; precio_unidad: string | null; imagen_url: string | null; pres_nombre: string | null; pres_nombre_plural: string | null; pres_factor: string | null; pres_codigo_barras: string | null; pres_gtin: string | null; pres_gs1_habilitado: boolean }
export type Presentacion = { id: number; producto_id: string; nombre: string; nombre_plural: string; factor_conversion: string; codigo_barras: string | null; gtin: string | null; gs1_habilitado: boolean; gtin_interno: boolean; activa: boolean; version: number; created_at: string; sku: string | null }
export type Lote = { id: string; producto_id: string; proveedor_id: number | null; numero_lote: string; fecha_vencimiento: string | null; costo_unitario: string | null; presentacion_id: number | null; recepcion_id: string | null; fecha_fabricacion: string | null; created_at: string }
export type Usuario = { id: string; nombre: string; email: string; whatsapp_phone: string | null; rol: string; activo: boolean; version: number; created_at: string; updated_at: string; deleted_at: string | null }
...
export type ProductoAreaRow = { id: string; codigo_interno: string; nombre: string; stock_maximo: string | null; punto_reorden: string | null }
export type ProductoAreaConfigInput = { producto_id: string; stock_maximo: string | null; punto_reorden: string | null }
...
export type CreateUsuario = { nombre: string; email: string; whatsapp_phone: string | null; password: string; rol: string; area_ids: number[] }
export type UpdateUsuario = { nombre: string | null; email: string | null; whatsapp_phone: string | null; rol: string | null; area_ids: number[] | null; version: number }
export type UsuarioResponse = { id: string; nombre: string; email: string; whatsapp_phone: string | null; rol: string; activo: boolean; areas: AreaSimple[]; version: number }
