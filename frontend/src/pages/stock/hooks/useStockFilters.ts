...
export type EstadoFiltro = 'todos' | 'normal' | 'bajo' | 'critico' | 'sin_stock' | 'vence_pronto'
...
function parseEstado(raw: string): EstadoFiltro {
  const valid: EstadoFiltro[] = ['todos', 'normal', 'bajo', 'critico', 'sin_stock', 'vence_pronto']
...
export function useStockFilters() {
  ...
  const { filters: sf, setFilters: setSf, clearFilters: clearSf, hasActiveFilters: hasSfActive } =
  ...
  const [estado, setEstadoState] = useState<EstadoFiltro>(parseEstado(estadoParam))
