...
export function useConteoSession(id: string | undefined) {
  ...
  const [localItems, setLocalItems] = useState<
    Record<string, { cantidad: string; estado: string; version: number }>
  >({});
  ...
  const editable =
    sesion?.estado === "borrador" || sesion?.estado === "en_progreso";
    ...
    () =>
      (data?.items ?? []).map((item) => {
        ...
        return {
          ...item,
          cantidad_contada:
            local.estado === "contado" && local.cantidad !== ""
  ...
  const stats = useMemo(() => {
    const contados = itemsConEdicion.filter(
      (i) => i.estado_item === "contado",
    ).length;
    const total = itemsConEdicion.length;
    const progreso = total > 0 ? Math.round((contados / total) * 100) : 0;

    const sinDiff = itemsConEdicion.filter(
      (i) =>
        i.estado_item === "contado" &&
        toDecimal(i.cantidad_contada).eq(i.stock_sistema),
    ).length;
    const negativo = itemsConEdicion.filter(
      (i) =>
        i.estado_item === "contado" &&
        i.cantidad_contada !== null &&
        toDecimal(i.cantidad_contada).lt(i.stock_sistema),
    ).length;
    const positivo = itemsConEdicion.filter(
      (i) =>
        i.estado_item === "contado" &&
        i.cantidad_contada !== null &&
        toDecimal(i.cantidad_contada).gt(i.stock_sistema),
    ).length;
    const noContados = itemsConEdicion.filter(
      (i) => i.estado_item === "no_contado",
    ).length;
  ...
  const guardarMutation = useMutation({
    mutationFn: (payload: GuardarConteoItemPayload[]) =>
      api.patch(`/conteo/${id}/items`, { items: payload }).then((r) => r.data),
    onSuccess: () => {
  ...
  const confirmarMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      notify.success(
        `Conteo confirmado: ${res.ajustes_generados} ajustes generados`,
      );
      navigate("/conteo");
    },
  ...
  const actions = useMemo(
    () => ({
      ...
      toggleNoContado: (item: ConteoItem) => {
        const estaNoContado =
          (localItems[item.id]?.estado ?? item.estado_item) === "no_contado";
        setLocalItems((prev) => ({
          ...prev,
          [item.id]: {
            cantidad: "",
