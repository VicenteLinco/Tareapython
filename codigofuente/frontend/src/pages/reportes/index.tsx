...
export default function ReportesPage() {
  ...
  useEffect(() => {
    if (productoActiveIndex >= 0) {
      productoItemRefs.current[productoActiveIndex]?.scrollIntoView({
        block: "nearest",
      });
    }
  }, [productoActiveIndex]);
  ...
  function handleProductoKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === "ArrowDown") {
      ...
      setProductoActiveIndex((i) =>
        i < productoSugerencias.length - 1 ? i + 1 : 0,
      );
    } else if (e.key === "ArrowUp") {
      e.preventDefault();
      if (productoSugerencias.length === 0) return;
      setProductoActiveIndex((i) =>
        i > 0 ? i - 1 : productoSugerencias.length - 1,
      );
    } else if (e.key === "Enter") {
      e.preventDefault();
      if (
        productoActiveIndex >= 0 &&
        productoSugerencias[productoActiveIndex]
      ) {
  ...
  const loading =
    calendarioQ.isLoading || productosQ.isLoading || descartesQ.isLoading;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between border-b border-base-300 pb-4">
        <div>
          ...
          <p className="text-sm text-base-content/60 mt-1">
            Consumo por dia, producto y descartes del periodo.
          </p>
        </div>
        <button
          ...
          <div className="relative">
            <input
              ...
              aria-activedescendant={
                productoActiveIndex >= 0
                  ? `reporte-producto-${productoActiveIndex}`
                  : undefined
              }
              onChange={(e) => {
      ...
      <div className="tabs tabs-boxed w-fit">
        <button
          className={`tab gap-2 ${tab === "calendario" ? "tab-active" : ""}`}
        ...
        <button
          className={`tab gap-2 ${tab === "productos" ? "tab-active" : ""}`}
        ...
        <button
          className={`tab gap-2 ${tab === "descartes" ? "tab-active" : ""}`}
          onClick={() => setTab("descartes")}
        >
          ...
          <div className="card-body gap-3 p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-xs text-base-content/50">
                {dias.length} {plural(dias.length, "dia", "dias")} con consumo
              </span>
            </div>
            {calendarioQ.isLoading ? (
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {dias.map((dia) => (
                  <article
                    key={dia.fecha}
                    className="rounded-lg border border-base-200 bg-base-50 p-3"
                  >
                    <div className="flex items-start justify-between gap-3 border-b border-base-200 pb-2 mb-2">
                      <div>
                        <div className="font-semibold text-sm">
                          {formatDate(`${dia.fecha}T00:00:00`)}
                        </div>
                        <div className="text-[11px] text-base-content/45">
                          {dia.productos}{" "}
                          {plural(dia.productos, "producto", "productos")} ·{" "}
                          {dia.movimientos}{" "}
                          {plural(dia.movimientos, "movimiento", "movimientos")}
                        </div>
                      </div>
                    </div>
                    <div className="space-y-2">
                      {dia.rows.map((row) => (
                        <div
                          key={`${row.fecha}-${row.area_id}-${row.producto_id}`}
                        ...
                        >
                          <div className="min-w-0">
                            ...
                            <p className="text-[11px] text-base-content/45 truncate">
                              {row.area_nombre} · ultimo {row.ultimo_consumo}
                            </p>
                          </div>
                  ...
                  <tbody>
                    {(productosQ.data ?? []).length === 0 ? (
                      <tr>
                        <td
                          colSpan={6}
                          className="text-center text-base-content/40 py-6 text-sm"
                        >
                          Sin consumos en el periodo
                        </td>
                      </tr>
