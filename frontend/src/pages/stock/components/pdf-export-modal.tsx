...
export function PdfExportModal({
  ...
  const [areaStatus, setAreaStatus] = useState<Record<number, AreaStockStatus>>(
    () =>
      Object.fromEntries(
        areas.map((a) => [a.id, "loading" as AreaStockStatus]),
      ),
  );
  ...
  useEffect(() => {
    ...
    Promise.all(
      areas.map((area) =>
        api
          .get<{ area: unknown; productos: unknown[] }>(
            `/stock/area/${area.id}`,
            { params: { per_page: 1 } },
          )
      ...
      setAreaStatus(
        Object.fromEntries(
          results.map((r) => [r.id, r.hasStock ? "con-stock" : "sin-stock"]),
        ),
      );
    });
  }, [areas]);

  const isLoadingStatus = Object.values(areaStatus).some(
    (s) => s === "loading",
  );
  const areasConStock = areas
    .filter((a) => areaStatus[a.id] === "con-stock")
    .sort((a, b) => a.nombre.localeCompare(b.nombre));
  const areasSinStock = areas
    .filter((a) => areaStatus[a.id] !== "con-stock")
    .sort((a, b) => a.nombre.localeCompare(b.nombre));
  ...
  const AreaChip = ({
    ...
    return (
      ...
      >
        ...
        {area.es_bodega && (
          <span className="text-[9px] font-bold uppercase tracking-wider opacity-40 shrink-0">
            BD
          </span>
        )}
        {checked && (
      ...
      <div className="bg-base-100 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-base-200 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 shrink-0">
          <div>
            ...
            <p className="text-[11px] opacity-40 mt-1 ml-10">
              PDF con stock por sección · carta horizontal
            </p>
          </div>
        ...
        <div className="overflow-y-auto px-6 pb-2 flex-1 space-y-5">
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex items-center gap-2 text-[11px] opacity-50">
              <span>
                {selectedIds.size} de {areas.length} seleccionadas
              </span>
              {!isLoadingStatus && (
                <span className="opacity-60">
                  ·
                  <span className="text-success ml-1">
                    {countConStock} con stock
                  </span>
                  {" · "}
                  <span className="ml-0.5">
                    {areas.length - countConStock} sin stock
                  </span>
                </span>
              )}
            </div>
            ...
            {isLoadingStatus ? (
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {areas.map((area) => (
                  <div
                    key={area.id}
                    className="h-9 rounded-xl bg-base-200 animate-pulse"
                  />
                ))}
              </div>
            ) : (
                ...
                {areasConStock.length > 0 && (
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[9px] font-bold uppercase tracking-widest text-success opacity-70">
                        Con stock
                      </span>
                    </div>
                    <div className="grid grid-cols-2 gap-1.5">
            ...
            <button
              ...
              className={cn(
                ...
                incluirResumen
                  ...
                  : "border-base-200 hover:bg-base-200/50 opacity-60",
              )}
            >
              <div
                className={cn(
              ...
              <div>
                ...
                <p className="text-[10px] opacity-50 mt-0.5">
                  Página inicial con KPIs: total de insumos, alertas y áreas
                </p>
              </div>
            </button>
          </div>
        ...
        <div className="px-6 py-4 border-t border-base-200 flex items-center justify-between shrink-0">
          ...
          <div className="flex gap-2">
            <button
              className="btn btn-ghost btn-sm h-9 px-4 font-bold"
              onClick={onClose}
            >
              Cancelar
            </button>
