...
export function LabelsSection({ detalles, onToggleEtiqueta, onCantidadEtiqueta, lotesConfirmados, onAfterPrint }: Props) {
  const [imprimiendo, setImprimiendo] = useState(false)
  const [collapsed, setCollapsed] = useState(false)

  // ── Fase post-confirmación (Estados de Configuración) ─────────────────────
  const [formato, setFormato] = useState<'rollo' | 'hoja'>('rollo')
  const [rolloTamano, setRolloTamano] = useState<'50x25' | '40x30' | '60x40' | '80x50' | 'personalizado'>('50x25')
  const [rolloAnchoCustom, setRolloAnchoCustom] = useState(50)
  const [rolloAltoCustom, setRolloAltoCustom] = useState(25)

  const [hojaTamano, setHojaTamano] = useState<'carta' | 'oficio' | 'a4'>('carta')
  const [hojaDiseno, setHojaDiseno] = useState<'3x10' | '3x8' | '4x10' | 'personalizado'>('3x10')
  ...
  if (lotesConfirmados) {
    ...
    const cols = hojaDiseno === '3x10' ? 3 : hojaDiseno === '3x8' ? 3 : hojaDiseno === '4x10' ? 4 : hojaColumnas
    const rows = hojaDiseno === '3x10' ? 10 : hojaDiseno === '3x8' ? 8 : hojaDiseno === '4x10' ? 10 : hojaFilas
    ...
    const sheetWidth = hojaTamano === 'carta' ? 215.9 : hojaTamano === 'oficio' ? 216 : 210
    const sheetHeight = hojaTamano === 'carta' ? 279.4 : hojaTamano === 'oficio' ? 330 : 297
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="card bg-base-100 border border-base-200 p-4 space-y-4 shadow-sm">
          {formato === 'rollo' ? (
            <div className="space-y-3">
              <p className="font-semibold text-xs text-base-content/50 uppercase tracking-wider">Configuración de Rollo</p>
              
              <div>
                <label className="label-text font-semibold text-xs text-base-content/80 mb-1 block">Tamaño de etiqueta</label>
                <select
              ...
              {rolloTamano === 'personalizado' && (
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="label-text font-semibold text-[10px] text-base-content/70 mb-0.5 block">Ancho (mm)</label>
                    <input
                      type="number"
                      min={20}
                      max={150}
                      ...
                      onChange={e => setRolloAnchoCustom(Math.max(20, Number(e.target.value)))}
                    />
                  </div>
                  <div>
                    <label className="label-text font-semibold text-[10px] text-base-content/70 mb-0.5 block">Alto (mm)</label>
                    <input
                      type="number"
                      min={15}
                      max={100}
                      ...
                      onChange={e => setRolloAltoCustom(Math.max(15, Number(e.target.value)))}
                    />
                  </div>
                </div>
              )}
              ...
              <label className="flex items-center gap-2 cursor-pointer mt-1 select-none">
                ...
                />
                <span className="text-xs text-base-content/85">Mostrar contorno de etiqueta</span>
              </label>
            </div>
          ) : (
            <div className="space-y-3">
              <p className="font-semibold text-xs text-base-content/50 uppercase tracking-wider">Configuración de Hoja</p>
              
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="label-text font-semibold text-xs text-base-content/80 mb-1 block">Tamaño de papel</label>
                  <select
                    className="select select-sm select-bordered w-full text-xs rounded-lg"
                    value={hojaTamano}
                ...
                <div>
                  <label className="label-text font-semibold text-xs text-base-content/80 mb-1 block">Distribución (Grilla)</label>
                  <select
                    className="select select-sm select-bordered w-full text-xs rounded-lg"
                    value={hojaDiseno}
                    onChange={e => {
                      ...
                      const newCols = val === '3x10' ? 3 : val === '3x8' ? 3 : val === '4x10' ? 4 : hojaColumnas
                      const newRows = val === '3x10' ? 10 : val === '3x8' ? 8 : val === '4x10' ? 10 : hojaFilas
              ...
              {hojaDiseno === 'personalizado' && (
                <div className="grid grid-cols-2 gap-2 bg-base-200/50 p-2 rounded-lg">
                  <div>
                    <label className="label-text font-semibold text-[10px] text-base-content/70 mb-0.5 block">Columnas</label>
                    <input
                      type="number"
                      min={1}
                      max={10}
                  ...
                  <div>
                    <label className="label-text font-semibold text-[10px] text-base-content/70 mb-0.5 block">Filas</label>
                    <input
                      type="number"
                      min={1}
                      max={20}
              ...
              <div className="grid grid-cols-2 gap-2 items-end">
                <div>
                  <label className="label-text font-semibold text-xs text-base-content/80 mb-1 block" title="Omitir las primeras N etiquetas si ya fueron usadas">
                    📍 Iniciar en posición
                  </label>
                  <input
                    type="number"
                    ...
                    onChange={e => setPosicionInicial(Math.min(totalSlots, Math.max(1, Number(e.target.value))))}
                  />
                </div>
                <div className="pb-1">
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      ...
                      onChange={e => setMostrarBordes(e.target.checked)}
                    />
                    <span className="text-xs text-base-content/85">Líneas guía de corte</span>
                  </label>
                </div>
              </div>
              ...
              <div className="border-t border-base-200 pt-2">
                ...
                {configAvanzada && (
                  <div className="grid grid-cols-4 gap-1.5 mt-2 bg-base-200/40 p-2 rounded-lg text-[10px]">
                    <div>
                      <span className="opacity-75 block mb-0.5">Marg. Vert.</span>
                      <input
                        type="number"
                        step={0.5}
                    ...
                    <div>
                      <span className="opacity-75 block mb-0.5">Marg. Horiz.</span>
                      <input
                        type="number"
                        step={0.5}
                        className="input input-xs input-bordered w-full text-center text-base-content"
        ...
        {formato === 'hoja' && (
          <div className="border border-base-200 rounded-xl p-3 bg-base-50 flex flex-col items-center shadow-inner">
                ...
                return (
                  <div
                    ...
                    title={`Posición ${idx + 1}: ${
                      type === 'skipped' ? 'Usada/Omitida' : type === 'printed' ? 'Etiqueta' : 'Vacía'
                    }`}
                  >
                    {type === 'printed' && (
                      <span className="text-[8px] scale-75 leading-none opacity-80">🏷️</span>
                    )}
                  </div>
                )
              })}
            ...
            <p className="text-[10px] text-base-content/50 mt-2 font-medium">
              Se {totalPages === 1 ? 'usará 1 hoja' : `usarán ${totalPages} hojas`} en total. ({printed} etiquetas).
            </p>
          </div>
        )}
        ...
        {/* Botón de Impresión */}
        <Button className="w-full btn-md text-sm font-bold shadow-lg" onClick={handlePrint} disabled={imprimiendo}>
          <Printer className="h-4 w-4 mr-2" />
          {imprimiendo ? 'Generando etiquetas…' : <span>Imprimir <CantidadConUnidad qty={total} unidad="etiqueta" pluralUnidad="etiquetas" /></span>}
        </Button>
      </div>
    )
  }
    ...
    <div className="card bg-base-100 border border-dashed p-4">
      ...
      {totalEtiquetas > 0 && (
        <p className="text-xs opacity-50 mt-2 text-right">
          <CantidadConUnidad qty={totalEtiquetas} unidad="etiqueta" pluralUnidad="etiquetas" /> se imprimirán al confirmar
        </p>
      )}
    </div>
  )
