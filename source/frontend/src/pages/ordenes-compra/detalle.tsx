...
export default function OrdenCompraDetallePage() {
  ...
  const totalSolicitado = items.reduce(
    (acc, i) => acc + i.cantidad_solicitada,
    0,
  );
  ...
  const canCancelar =
    isAdmin && (oc.estado === "borrador" || oc.estado === "enviada");
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm opacity-50 mt-0.5">
            {oc.proveedor_nombre} &bull; {oc.usuario_nombre} &bull;{" "}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-base-100 border border-base-200 rounded-2xl p-4">
          <p className="text-[10px] uppercase font-bold opacity-40 tracking-widest mb-1">
            Solicitud origen
          </p>
          {oc.solicitud_numero ? (
        ...
        <div className="bg-base-100 border border-base-200 rounded-2xl p-4 col-span-2 md:col-span-1">
          ...
          <p className="text-[10px] opacity-40 mt-1">
            {totalRecibido} / {totalSolicitado} unidades
          </p>
        </div>
        <div className="bg-base-100 border border-base-200 rounded-2xl p-4">
          <p className="text-[10px] uppercase font-bold opacity-40 tracking-widest mb-1">
      ...
      {oc.nota && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-sm text-amber-800">
          <span className="font-bold text-[10px] uppercase tracking-widest block mb-1 opacity-60">
            Nota
          </span>
          {oc.nota}
        </div>
        ...
        <table className="table w-full">
          <thead>
            <tr className="bg-base-200/50 text-[11px] uppercase tracking-wider opacity-60">
              ...
              <th className="text-right hidden md:table-cell pr-6">
                Precio unit.
              </th>
            </tr>
          </thead>
            ...
            {items.map((item) => {
              const itemPct =
                item.cantidad_solicitada > 0
                  ? Math.round(
                      (item.cantidad_recibida / item.cantidad_solicitada) * 100,
                    )
                  : 0;
              const isComplete =
        ...
        <div className="bg-base-100 border border-base-200 rounded-[2rem] overflow-hidden shadow-sm">
          ...
          <ul className="divide-y divide-base-200">
            {recepciones.map((rec) => (
              <li
                key={rec.id}
                className="px-6 py-3 flex items-center justify-between hover:bg-base-200/20 transition-colors"
              >
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs text-base-content/50 hidden sm:block">
                    {formatDate(rec.fecha_recepcion)}
                  </span>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSelectedFotoTitle(
                          rec.guia_despacho || rec.numero_documento,
                        );
                      }}
                    >
                  ...
                  <EstadoBadge
                    estado={
                      rec.estado === "completa" ? "confirmada" : rec.estado
                    }
                    size="sm"
                  />
                </div>
